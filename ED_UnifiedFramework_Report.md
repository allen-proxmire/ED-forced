...
date: "July 2026 (currency pass 2026-09-07)"
---
...
| **Entanglement (Bell / Tsirelson)** | ✅ closed arc (non-factorizability, monogamy **as a finite-budget cap** — not the CKW form — no-signaling forced, the Bell–Tsirelson bound); read as the pre-individuation regime, not action-at-a-distance | commitment (P11) individuates; entanglement is the not-yet-committed regime | Closed (Grounded/Derived) + interpretation | §4 |
...
| **Novel falsifiable predictions** | ✅ ranked weapons + one strong postdiction (the degenerate-mobility law *shape*, §15 — matches existing data across ten materials, not peer-reviewed, not a forward test); the flagship distinctive bet (`a₀(z) = cH(z)/(2π)`) has met first data — evolution seen, constant `a₀` excluded decisively (ED's call confirmed, MOND's killed), exact rate mildly disfavored (data run faster than `H(z)`; **2.3σ on a computed conversion budget**, survey systematics on top and unquantified); ⚠️ no argument-ending weapon fully confirmed | several sharp falsifiers trace to the arrow (`a₀(z)`, w=−1, the clean-vector prohibitions) | Falsifier frontier | §15 |
...
- **📏** — inherited by design. ED's own logic (no intrinsic scalar) says it cannot and need not set this. Not a hole. *(the comparison with the Standard Model's own parameter inheritance is made once, in §12; it is referenced rather than restated here)*
...
ED has a **closed entanglement arc** (Papers 063–070): composite states are non-factorizable (the tensor-product structure), the Schmidt decomposition follows **(conditional on `P-Bipartite-Mapping` + `P-V5-Schmidt-Generic`)**, **monogamy** comes from the finite cross-chain bandwidth budget — **what is derived is the budget cap `Σᵢ ℰ ≤ W_max`; the CKW *form* was re-tiered 2026-09-04 to “consistent with, not forced by” it** — no-signaling is **forced** on three locks, **though the “over-determined” verdict is `Paper_066`'s own `A→position` rather than a derived result**, von Neumann entropy is grounded, and the **Bell–Tsirelson polytope** (`Bell ⊊ Tsirelson ⊊ NS`, placing ED exactly at the quantum ceiling with PR-boxes excluded) is *form-forced* — conditional on V5 enforcing a Hilbert-space inner-product structure (`Paper_069`) — while its numerical value `2√2` is inherited as standard mathematics, not derived from primitives. All of it is carried by **V5**, the cross-chain kernel: the finite-reach correlation between distinct chains. (Tier: the arc is closed; no-signaling is Derived-forced; the Tsirelson polytope is form-forced conditional on the V5–Hilbert postulate, its `2√2` value inherited.)
...
- In its **deep field**, it is the dark-matter phenomenology. KM-I embeds MOND relativistically as the khronon's low-acceleration behavior, forced given the ED combination rule `a = √(a_N·a₀)` (Paper_030), with the transition scale `a₀ = cH₀/(2π)` coming from the cosmic decoupling surface (Paper_029) — **the *scale* `a₀ ~ cH₀` is derived, by three independent routes; the `1/(2π)` *coefficient* is re-tiered Postulated/disputed as of 2026-09-04 and is not established as forced** —, the slope-4 Baryonic Tully–Fisher relation `v⁴ = GMa₀` with zero intrinsic scatter (Paper_031), and the AQUAL field equation (Paper_036). Lensing and PPN pass, and there is no new ghost (the `W`-sector adds no time derivatives); but KM-I is explicit that its feedback into the kinetic matrix is the same regulator degeneracy the cosmological sector below leaves unfit, so the full strong-coupling stability is conditional, not a closed proof (KM-I, Grounded/conditional). ED does not add dark matter; it reads the rotation curves off the same field that gives Newton. **One internal tension, flagged rather than smoothed over: `Paper_KM-I` and `Paper_030` handle the HIGH-acceleration limit by two different and mutually uncited mechanisms** — `Paper_030` §7.1 uses a **switch** (three regimes, the cross-term inactive above `a₀`), `KM-I` a **smooth Cassini-constrained family**. **The finding is not in `KM-I`**, which is careful and says outright that it carries `Paper_030`'s law rather than re-deriving it; but the corpus has since **derived** the switch, and this section's *“standing MOND result”* framing carries no trace of the disagreement. **Neither account is falsified and the phenomenology is unaffected; which one is canonical is an open call for the author** (gravity ledger #19).
...
- **The MOND scale evolves with redshift, `a₀(z) = cH(z)/(2π)`.** *(The **evolution** is the claim here, and it is **form-forced conditional on the live-horizon reading** — the scale `~cH` is multiply supported (three routes), while **the live-versus-frozen horizon identification is the remaining physical premise and is not a theorem**. The workbook re-tiered this row `Derived → Grounded` on 2026-09-07 for exactly that reason, and this sentence is synchronised to it. — `Paper_029` audit row 14 states the exponent-1 result is explicitly **independent of the `1/(2π)` coefficient dispute**. Only the coefficient inside the formula is disputed.)* Because `a₀` is tied to the cosmic horizon `R_H = c/H`, it tracks the instantaneous Hubble rate rather than being a constant of nature, so it grows with redshift (×1.8 by `z = 1`). The power `α = 1` is forced not by dimensional analysis (ED has a second scale, `ℓ_P`, so a bent power is dimensionally legal) but by the projection mechanism plus ED's no-intrinsic-scalar result (§12), which leaves no dimensionless number free to bend it at the fixed magnitude, so ED cannot fit `a₀` the way MOND does. It is the sector's most distinctive live weapon: first data (MUSE-DARK III, 2026) confirm `a₀` evolves and exclude a constant `a₀` decisively, matching ED's local value to ~8%, while mildly disfavoring ED's exact rate (the observed evolution runs somewhat faster than `H(z)`, a few-σ tension from one first-generation survey). This is the flagship of §15.
...
**It is closed in the GR regime that has been tested, and the cosmological extension is not** *(wording corrected 2026-09-07: “a closed sector” overstated it, and the list immediately below said so)*. The edges:
...
The galactic phenomenology of §5 has a cleaner reading than "a modification bolted onto gravity." Reading gravitational strain as **quadratic in the participation amplitude** — `|Σ_a P^(a)|²` with `P = √b · e^{iπ}` — splits it into a diagonal and an off-diagonal term, **and the two halves land in different terms of `Σ_C = Coh − Str − Grad`: the diagonal is `Str`, the off-diagonal is `Coh`.** **That split is what makes both accelerations attractive, and it is settled rather than chosen** — the Newtonian piece goes as `1/R` and the MOND piece as `ln R`, so `d(1/R)/dR < 0` while `d(ln R)/dR > 0`, and both can only come out attractive if the two enter `Σ_C` with **opposite signs**, which a single squared modulus cannot supply. Of three candidate assignments exactly one reproduces `a = a_N + √(a_N a₀)`, to floating-point precision at every radius (`internal notes/_check_sigma_sign.py`, re-runnable; foundations ledger #10, 2026-09-05). **P12 needed no amendment.** **The diagonal is Newton** (each source's self-potential); **the off-diagonal is MOND** — the interference *cross-term* between the local mass and the cosmic horizon, whose modulus is exactly the geometric mean `√(a_N · a₀)`, with `a₀` supplied by the horizon **(the scale `~cH₀` derived; the `1/(2π)` coefficient Postulated/disputed, 2026-09-04)** (Paper_QuadraticStrain, sharpening Paper_030; Paper_029). This is the same interference structure as the double-slit, and it *discharges* the bilocal-coupling postulate (P14) the MOND line previously carried: the `√` is forced **given `P-Quadratic-Strain`** — the geometric mean is the unavoidable off-diagonal of `|P^(L)+P^(H)|²`, while the alternatives Paper_030 could not exclude are the *diagonal* self-terms, a different part of the same expansion (Grounded / structural). **One correction the arc made to itself: the companion claim that the *regime assumption* was also discharged is WITHDRAWN** — a third branch was tried and breaks MOND, giving `a ∝ GM H₀/R²`, so `Paper_030` §5.3's regime assumption is **restored** and *“two postulates to one”* reads **one and a half**: the P14 discharge stands and is the strong half. **And the withdrawal surfaced what the arc calls arguably its deepest open question: `Paper_029` gives a horizon response first-order in `|a|`, while MOND requires a threshold at `a₀` — how does a linear response produce a threshold?** Neither paper asks it; it was invisible while `Σ₀` was carried as a constant source (gravity ledger Staleness #13). **The MOND residuals — CORRECTED 2026-09-07 after a physics audit, because the list published here that morning overstated them** (`gravity/Note_PhysicsAudit_MONDResiduals_2026-09-07.md`). **(i) The threshold has a CANDIDATE MECHANISM, dated 2026-09-04, that answers it structurally with no new postulate**: a chain has two saturation boundaries, its own at `R_a = c²/a` and the cosmic one at `R_H = c/H₀`, and **only the nearer binds** — so the handover sits at `a = cH₀`. `Paper_029`'s linear response is untouched and describes behaviour *within* a regime; what changes at the threshold is **which boundary saturates**. *It is the third independent route to the scale `cH₀`, and it has not been promoted from candidate.* **(ii) The high-acceleration switch versus a smooth family is NOT an open derivation** — `μ`'s shape cannot be derived without the V5 envelope shape, which `Paper_090` §7.2 lists as **value-layer inherited**; the corpus's own note calls it *“an inherited quantity that has been mis-filed as an open one”*, and the arc already carries the analogous disclaimer for the V1 kernel. **(iii) The universal-free-fall normalisation (§14) is the one genuine open derivation of the three — and it narrowed to a single exponent on 2026-09-07** (`gravity/Note_UFF_TheExponentIsWrong_2026-09-07.md`). **The drive is a cross term, so it carries the test chain's own `√b_C`; P12 as literally stated adds no normalisation, giving `a_C ∝ √b_C` — a heavier chain falls faster.** **The repair on record, `a_C = (1/b_C)[−∇_adj Σ_C]`, does NOT fix it:** it gives `a_C ∝ b_C^{−1/2}`, **the same violation inverted** (over a 256× bandwidth range both spread by 16×). *It had been assessed on whether amending a primitive is affordable, never on whether it is sufficient.* **Demanding `b_C`-independence fixes the power uniquely at `1/√b_C` — the chain's own AMPLITUDE, since `P = √b e^{iπ}`** — which is the natural normalisation for a drive that is amplitude×amplitude: dividing by one's own amplitude leaves exactly one factor of the *source's*. **Whether the substrate normalises that way is open, but it is now one testable exponent rather than an unbounded gap.** **The constructive sign is not among them** — settled structurally, with only its measured corroboration outstanding. So ED does not add a MOND force — MOND is what the interference of matter *with the horizon* looks like, and Newton is the same strain form's diagonal. **The residual here is no longer “the constructive sign,” and saying so was stale** *(corrected 2026-09-07)*. **The sign is structurally settled:** under the assignment fixed by the three-way check (foundations #10), `Coh` enters `Σ_C` with a plus and MOND is attractive, so `cos Θ_LH > 0` **follows from the assignment** rather than waiting on a measurement. **What remains candidate is its independent measured corroboration**, and that is a different and smaller claim: the run behind the *Measured* sign claim implements `|acc|/n`, which carries no neighbour-to-neighbour cross term, so **it measured `Grad`'s phase behaviour, not `Coh`'s** — the sign transfers only because `+Coh` and `−Grad` both reward alignment and differ by that term. **The corpus's own row status is “CANDIDATE — confirm”, and this report should be read at that tier until it closes** (gravity ledger #112, 2026-09-06).
...
**⭐ The flagship: the redshift evolution of the MOND scale, `a₀(z) = cH(z)/(2π)`.** Because ED ties `a₀` to the cosmic horizon `R_H = c/H`, the MOND acceleration scale is not a constant of nature (as in MOND) but tracks the instantaneous Hubble rate, so it must grow with redshift, roughly ×1.8 by `z = 1` and ×3 by `z = 2`. The power `α = 1` is forced — though, to be precise, not by dimensional analysis: ED carries a second length scale (`ℓ_P`), so `a₀ = c·H·(ℓ_P H/c)^p` is dimensionally legal for any `p`. What fixes `p = 0` is the projection mechanism (Paper_029's dipole projection introduces no `ℓ_P`) together with ED's no-intrinsic-scalar result (§12): with the ~10% magnitude match already in hand, ED has no order-spanning dimensionless number available to bend the power. So `α = 1` is mechanism-forced, and ED cannot fit `a₀` the way MOND fits its constant. The prediction is genuinely forward-dated — it is in Paper_038's cosmological section two months before the data, with non-evolution named as a refutation — with one caveat of record: a second corpus paper (Paper_031) disclaimed the evolution prediction until that internal contradiction was reconciled on the day the data was confronted. And the first data exist. MUSE-DARK III (A&A 2026) measures the radial-acceleration-relation scale across `0.33 < z < 1.44` and finds it evolves at ~30σ, with `a₀` rising from `1.2` locally to `2.38 (+0.12/−0.10) × 10⁻¹⁰ m/s²` by `z ~ 1`, excluding a constant `a₀` decisively. **Stated precisely, because the looser phrasing outran the analysis:** the data favour an evolving `a₀` and **exclude constant-`a₀` MOND** — but **evolution itself does not distinguish ED from ΛCDM or from an evolving-`a₀` MOND with a fitted exponent.** ED's distinctive test is `α = 1`, which remains open and under tension. Matching ED's local value to ~8% — though the `2π` re-tiering means that ~8% rests on a coefficient nothing upstream fixes, making it a consistency remark rather than evidence. **And the detection of evolution does not, by itself, separate ED from ΛCDM** *(added 2026-09-07)*: the **Magneticum** hydrodynamical simulations (*MNRAS* `10.1093/mnras/stac3017`, “ΛCDM with baryons versus MOND: the time evolution of the universal acceleration scale”) find that plain ΛCDM **with baryons** produces an *apparent* rise in the RAR scale with redshift, from ordinary galaxy assembly and baryon modelling, with no modified gravity at all. What the ~30σ excludes is **constant-`a₀` MOND specifically**; ΛCDM, an evolving-`a₀` MOND with a *fitted* exponent, and ED all accommodate evolution and separate only on the **rate** — which is precisely the part under tension. One tension remains, on the rate, and it is the survey's *own* headline conclusion: it fits a *linear* `a₀(z)` and reports the evolution running **faster than `H(z)`** — which is directly the anti-ED direction, since ED is forced to exactly `H(z)`. Recasting the survey's measured points as a power law gives `α ≈ 1.18` — an ED-side re-fit, *not* a figure MUSE-DARK III itself reports — which places ED's forced `α = 1` a nominal ~4σ from that recast (its ±0.04 an underestimate — **and now quantified: a full conversion budget gives `α = 1.15 ± 0.07`, putting `α = 1` at 2.3σ, with the intercept, the cosmology and the reduction choice supplying 73% of the variance the published bar omits; one-survey astrophysical systematics are on top of that and unquantified**). **A finding from doing the refit rather than asserting it: their linear fit has no constant exponent — its local slope runs 1.78 at `z = 0.33` down to 0.90 at `z = 1.44`, crossing ED's `α = 1` at `z ≈ 1.1`, inside the data.** MOND's `α = 0` is dead regardless. **⚠ And a caveat added 2026-09-07 that outranks all of the above: a re-analysis of MUSE-DARK III's own public catalogue** (Rusishvili 2026, self-posted and **not peer-reviewed**) **reports three baryonic-mass-budget systematics, each claimed sufficient to produce the trend — and their effect sizes are quantitatively coherent with published numbers.** In the deep-MOND regime `a₀ = g_obs²/g_bar`, so an underestimated baryon budget inflates the inferred `a₀` linearly; molecular gas is omitted from the survey's model and its fraction rises with redshift, which makes `a₀` appear to evolve when it is constant. **This attacks the ~30σ detection, not merely the rate.** If it holds, ED's qualitative win and the 2.3σ tension **fall together**, constant-`a₀` MOND is no longer excluded either, and the honest status of the flagship is **UNTESTED** rather than confirmed or refuted. **It is not banked in either direction.** **✅ And it is decidable rather than merely arguable:** the survey's own text confirms the molecular-gas omission (`a_bar` carries disk + HI + bulge only, with the authors' own **~0.2 dex** attached to it against a **0.334 dex** trend), **and the full catalogue is public** at `dark.univ-lyon1.fr/data-releases/`. The decisive test is **executable**, and until someone runs it this prediction is untested rather than confirmed or refuted. So ED's qualitative call is confirmed and MOND's excluded, while ED's exact rate is disfavored but not refuted. The decisive test is a direct, multi-survey `a₀ ∝ H(z)^α` fit; `α = 0` is already dead, and ED lives or dies on `α = 1`. This is the most distinctive, most live bet in the document.
...
- **The Universal Degenerate-Mobility law** `D(c) = D₀(1 − c/c_max)^β`. ED derives the *functional shape* blind to the data (the ED PDE reduces to the porous-medium equation); the exponents (`β`, `c_max`) are fit per material (`Paper_085`: "matching, not derivation"; population `β = 1.72 ± 0.37`), and the shape holds across ten materials at `R² = 0.986`–`0.999`. So it is a *shape* matched to existing published data — a strong **postdiction** (the ten datasets predate the derivation), documented in a self-authored Zenodo preprint (**not peer-reviewed, not independent validation**), not a confirmed number and not a forward test. It is the corpus's strongest data-match at that tier; the genuine forward leg (the FRAP front-radius exponent) is not yet run.
...
Most importantly: **no distinctive, argument-ending prediction is confirmed** (§15). ED has one strong postdiction (the *shape* of the degenerate-mobility law, derived blind and matched to existing data; its exponents are fit; a self-published preprint, not peer-reviewed) and survives the tests that killed rival theories (`c_GW = c`). Its flagship distinctive bet, the redshift evolution of the MOND scale `a₀(z) = cH(z)/(2π)`, has now met its first data and returned a *partial* result: the evolution is real and a constant `a₀` is excluded decisively, so ED's qualitative call is confirmed and MOND's is killed, but the observed rate is somewhat faster than ED's forced `α = 1`, a mild tension — **2.3σ on a computed conversion budget (`α = 1.15 ± 0.07`), superseding the earlier “nominal ~4σ, ~1–2σ under systematics”, which was asserted without a budget** — from a single first-generation survey **whose ~30σ detection is itself under a coherent systematics challenge (§15).** That is a live, distinctive confrontation, not a confirmation. The other bets that would single ED out cleanly — the universal envelope ratio, the step-like cluster knee, the architecture-independent decoherence wall — are still open. Until one of these confirms, ED is a coherent, falsifiable unified framework, not a certified one. That distinction is the ceiling of this entire document.
