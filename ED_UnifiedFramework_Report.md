...
| **Novel falsifiable predictions** | ✅ ranked weapons + one strong postdiction (the degenerate-mobility law *shape*, §15 — matches existing data across ten materials, not peer-reviewed, not a forward test); the flagship distinctive bet (`a₀(z) = cH(z)/(2π)`) has met first data — evolution seen, constant `a₀` excluded decisively (ED's call confirmed, MOND's killed), exact rate mildly disfavored (data run faster than `H(z)`; ~1–2σ with systematics); ⚠️ no argument-ending weapon fully confirmed | several sharp falsifiers trace to the arrow (`a₀(z)`, w=−1, the clean-vector prohibitions) | Falsifier frontier | §15 |
