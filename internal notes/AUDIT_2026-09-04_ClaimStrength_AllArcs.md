...
| **soft-matter** | **— (ledger built #55)** | **STILL NOT AUDITED, but now auditable.** Found 2026-09-04 (Staleness #54): the arc had 17 papers and **no tiered-claims ledger**, the only content arc without one. This pass swept *ledgers*, so an arc without one was never in the set — **the coverage claim above was measuring the instrument, not the corpus.** A ledger was built the same day (#55) from each paper's own audit table, so the arc is now visible. **It has still had no claim-strength pass**, and that is the outstanding item. |
