...
**3. `Grad` remains a proposal — TIER REVISITED 2026-09-06 (#113): still not derived, but now SIX supports (two dynamical) rather than four, the recorded (A)-crystallization problem reattributed as not `Grad`-specific and in fact worse for `Coh`, and a new caveat — **extensive `Grad`, which is how it appears in `Σ_C`, does NOT bind; only intensive scoring does**, so the new support is for `Grad`'s phase STRUCTURE and not for its extensive appearance. **Opens a question nobody has asked: is `Σ_C`'s extensive `−Grad` the right way for the term to enter at all?** — STILL A PROPOSAL, and its one surviving discriminator was WITHDRAWN 2026-09-06 (#103):** the `ξ`-versus-coupling trend was a **coordination-number artifact** (a phase-blind control reproduces it). **The structural reason is now stated: `Coh` sums over contribution-pairs at a locus, `Grad` over graph edges, and a one-phase-per-node probe collapses those onto each other — so no probe of that design can settle it. A discriminating probe needs multiple channels per locus.** — **AND THEN, SAME DAY (#104): the probe's `Coh` arm was never `Coh`.** `|acc|/n` is the simulator's per-locus-average convention; canonical `Coh` is `2|acc|` **plus a neighbour–neighbour cross term**. **So the comparison was `Grad` against `Grad/n`, and the question was never asked** — which explains the draw, the `ξ` artifact, and why nothing helped. **`Coh = −Grad + 2·NN + const`: they differ by exactly one object, the neighbour–neighbour term, which a Dirichlet form structurally cannot have.** **Cheapest real next step: re-run the original probe with its `Coh` arm corrected to `|acc|² − n`.**** Six supports, one passed compatibility check, one failed Knots sub-case, not derived. And an open question about whether `Paper_PhaseCoherence`'s measured phase-alignment belongs to `Grad` rather than `Coh`.
