...
25. □ **Settle a disputed upstream choice by asking which one reproduces the arc's established result.** *(Added 2026-09-05.)* When two readings of a definition are both defensible, **do not pick the one that reads more naturally** — push each through to a quantity the corpus already has and see which lands on it. **Instance:** `|ΣP|²` was assigned to both `Coh` and `Str`, which enter `Σ_C` with opposite signs. I recommended the reading that seemed likeliest; **it turned out to be unavailable**, both papers describing the same index set in the same words. A three-way check against `a = a_N + √(a_N a_0)` settled it in one run, to floating-point precision, and **the winner was a reading I had not favoured.** **Corollary:** an overall sign silently absorbed into a definition (`Φ_N = Σ_K b_K` swallowing P12's `−Str`) hides collisions indefinitely — **check every definitional equation against the sign conventions of the primitives it draws on, and against P04-style non-negativity.** `internal notes/_check_sigma_sign.py`; gravity ledger Staleness #71.

24. □ **Open the source you cite for a definition, and check the definition is in it.** *(Added 2026-09-05.)* **The soft-matter arc cited `Paper_073_DCGT` for `a_eff` — a symbol that appears nowhere in that paper**, and the quantity DCGT actually defines (`R_cg`) is a **chosen** scale that the paper says results are *insensitive to*, which makes it unusable as the physical floor the arc needed. The citation had stood since the paper was written. **Two related habits.** **(a) When a chain reads** *“X, coarse-grained to scale Y, therefore a floor at Y”* **— check that step; coarse-graining removes structure below a cell, it does not erect a barrier at it.** **(b) A falsifier marked** *“consilience pass”* **has usually been checked against the confirming half of a literature only.** `F-EXIST` was passed that way and its condition was met all along. **Re-check every consilience pass in the corpus on the same suspicion.** Gravity ledger Staleness #66.

23. □ **Name your falsifiers, and put the name in the master predictions list.** *(Added 2026-09-05.)* `F1`/`F2` numbering is per-paper and joins nothing; **a named `F-NAME` is the join key between a paper and `ED_Master_Predictions_List.md`.** Without it the list cannot be navigated from the papers, which is how the soft-matter transport arc stayed out of the inventory from the day its paper was written. **Run `python "internal notes/_census_postulates.py" --predictions` after any session that adds a falsifier**; it exits 1 on a newly orphaned one. **Clear a hit by adding the label to the relevant master-list row, never by deleting it from the paper.** The same run also flags papers whose own prose calls their falsifier content *distinctive* while the list has no row for them — which is how ED's warm-relic and no-galactic-halo bets were found to be absent from ED's own prediction inventory. Gravity ledger Staleness #63.

22. □ **Comparative claims about other literatures get a web check before they get written.** *(Added 2026-09-05, failure mode 8.)* Any sentence saying a rival framework lacks something, or that ED is first, is **unverifiable from inside the corpus**. Search before writing; one search is cheaper than the five-site correction the last one needed. Three rules. **(a) Never write** *“nobody has”* — a negative-existence claim about a literature cannot be verified even in principle, so the honest form is *“we have not found”*. **(b) Prefer the narrow form**, which is almost always the stronger claim: not *“no MOND analogue”* but *“MOND has the relation with no derived normalization, so its inversion returns only an order of magnitude”*. In every correction to date the narrow version was sharper, and twice it turned a dead assertion into a computable open item. **(c) Never invent a section or equation number** to make a citation look pinned; record *“pin before submission”*. **Instances:** *“this probe does not exist in MOND”* (false — Milgrom 2020 is a review of exactly that connection; had propagated to five sites including the public workbook); *“standard physics has no reason to unify them”* of MIR and the viscosity floor (false — Zaanen 2019, and the same paper said the opposite three lines earlier); *“no MOND-EFE variant naturally produces it”* (MOND is nonlinear and modified-inertia variants are non-local). Sweep with `_sweep_comparative_claims.py`. **Item 22 runs in BOTH directions** *(added 2026-09-05, after F-CONVERGE)*. The outward form is a false claim *about* a neighbouring literature. **The inward form is an unchecked assumption that a neighbouring literature's phenomenon exists in the shape your derivation needs**, and it is the more expensive of the two because a whole arc can be built on it. **Instance:** the soft-matter transport arc reads its wall as MIR resistivity saturation and points at strange metals — **the one material family where resistivity does not saturate at all** (Gunnarsson, Calandra & Han, RMP 75, 1085). Nobody inside the corpus could have caught it; four instruments built the same day would not have. **Before an arc rests on an experimental phenomenon, check that the phenomenon occurs in the materials the arc names.**

21. □ **Re-tier propagation — a ledger entry is not propagation — **and a ledger's own prose summary is itself a propagation target** (added 2026-09-04, third instance: an arc-state paragraph still listed as a live exception an item the same ledger's staleness log recorded FIXED two months earlier). After fixing a flagged defect, grep the arc ledger's `### Honest arc-state` prose for the flag, not just the staleness row.** *(Added 2026-09-04.)* When a claim is re-tiered, downgraded or disputed, the change is **not complete** until: (a) a corpus-wide grep for the claim's **distinctive phrasing** has been run — not the claim's *topic*, its wording, since that is what propagated; (b) every hit is either flagged inline or confirmed as not asserting the claim; and (c) the **public-facing** documents are checked first, since they are quoted most and read by the people least able to check. **Instance:** the a₀ `1/(2π)` was re-tiered to *Postulated / disputed* on 2026-09-04 and recorded in the arc ledger and the two source papers. It stood unflagged in **twelve** other documents, including the whitepaper, the program review and the framework position paper, for the rest of that day. **The re-tier had been recorded and not propagated, which is the same defect the read-first rule exists to prevent, one level up.** **Third face, added 2026-09-04: a *research target* is a propagation target too.** When a prediction's status changes, update every target that recommends it in the same pass. A target doc is where a reader goes to decide what to **do**, so a stale one costs a session rather than a sentence. Found when research target #16 recommended the second-harmonic signature as a near-term test that `ED_Master_Predictions_List` 4.2 had marked **CONFOUNDED on the same day** — and the mismatch surfaced only when someone acted on it, not when the lists were read against each other.
...
20. □ **Strength audit — does the sentence claim more than the chain delivers?** *(Added 2026-09-04 after six audits found the same defect.)* Three passes, all cheap:
   ...
   - **(d) Endpoint eliminations.** Where a claim is reached by *refuting an alternative*, check that the alternatives are **exhaustive** and not the two ends of a continuum. The recurring form is: option A is refuted, therefore option B — where A and B are limiting cases of a one-parameter family and the interval between them is never addressed. **The grep will not catch this**; it needs reading. Three instances as of 2026-09-04, all in otherwise-honest papers: `GR-III` §4 refutes `β = 1` and needs `β = 0` **exactly** (Staleness #12); `Paper_038_5` §3.5 refutes live-tracking `ρ_Λ ∝ H²` and asserts the frozen floor, i.e. `n = 1` refuted and `n = 0` claimed, with the whole observationally allowed range `|n| ≲ 0.03` sitting in the untouched interval (#24); `GR-IV` §3 states `α₂ = 0` exactly on an input its own source gives at leading order (#20). **Test:** write the family with an explicit parameter, mark where the refuted case sits, where the claim sits, and where the data sits. If the data sits between them, the elimination is not doing the work the prose says it is.
