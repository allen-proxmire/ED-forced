...
- **The exact rate is in mild tension.** **Recasting the survey's points as a power law** gives $α ≈ 1.18 ± 0.04$ (**an ED-side re-fit; MUSE-DARK III publishes a *linear* $a_0(z)$ with $a_1 = 1.59±0.1×10^{-10}$, not a power law**); the data run somewhat *faster* than `H(z)`. ED's forced $α = 1$ sits ~4.5σ off the raw fit — but that bar propagates only their $a_1$ error through one reduction. **A full conversion budget gives $α = 1.15 \pm 0.07$, i.e. $α = 1$ at 2.3σ**, with the intercept, the cosmology and the reduction choice supplying 73% of the variance. The survey's own systematics are on top of that and unquantified. **And their linear fit has no constant exponent at all: its local slope runs 1.78 → 0.90 across their range, crossing $α = 1$ at $z \approx 1.1$.**
...
| $a₀(z) = cH(z)/(2π)$ evolves | 7 | Prediction; ≈30σ evolution confirmed (2026), local value ~8% (but on a disputed coefficient), exact rate 2.3σ tension on a computed budget |
