...
## 5. Inconsistencies found — one resolved, two recorded
...
2. **`b_int` means two different things — found 2026-09-06, unresolved.** **Individuation:** `b_int(S)` = the sum of `w(e)` over edges with **both endpoints inside the complex `S`** — an edge sum over a region (`foundations/Paper_Individuation_TheSystemEnvironmentCut` §1). **The Internal band:** `b_int(C)` = bandwidth along **`C`'s own update-rule participation** — a channel-class total (`concepts/participation_bandwidth.md`). **They coincide if a chain's own-rule channels are exactly the edges internal to it, which is plausible and unargued. Nothing in the corpus states either way**, and `Paper_GR-I`'s `P-Commitment-Linear` uses the second sense.
3. **`Grad` sits on the same graph.** The P12 card defines `Grad` as *"the slope of event density across participation-adjacency neighborhoods"* — sense **A**, and the same domain as **D**. That is consistent, but it means P12 carries **two** adjacency-flavoured objects, and a reader meeting either should check which is meant.
