...
- **DIMENSION, declared 2026-09-08: `[b] = 1` — bandwidth is DIMENSIONLESS.** *Canonical `Paper_087` §P04 gives `b ∈ ℝ_{≥0}` and stops, and almost every downstream use is a **ratio** (`N_bw`, `θ_ind`, `M_eff`, the Born rule) — all **homogeneous of degree 0**, so they constrain nothing.* **The constraint comes from `Paper_GR-I`'s lapse identification `N² ∼ b`:** the lapse is dimensionless (`g₀₀ = −N²`), and this is **the only place in the corpus where `b` is equated to something whose dimension is independently fixed.** *Consistent with `P-Commitment-Linear`'s `Γ ∝ b`, whose proportionality constant then carries `1/T`.* **Consequence: `P_K`, `Coh`, `Str` and `Σ_C` are all dimensionless too** — one declaration, five symbols. *(Foundations ledger, declarations pass U1.)*
