...
`internal notes/` (18 files — **`POSTULATE_BASIS.md` added 2026-09-04: the answer to “how much does ED assume?” — a basis ladder (171 declared / 26 cross-cutting / 103 used exactly once) with the counting rule stated and the recommended public wording; research target #20. `_census_postulates.py` added 2026-09-04: the standing postulate-count check (re-runnable, declares its basis, exits nonzero on drift from its baseline of 173; see research target #20). `AUDIT_2026-09-04_ClaimStrength_AllArcs.md` added 2026-09-04: the single-session claim-strength audit of the fifteen arc ledgers that existed at the time — **`soft-matter` was not among them, having had no ledger** (one was built 2026-09-04; see that arc's ledger, staleness #1); coverage table, findings, the open list, and the targeting model. Map only — the full record is `physics-papers/gravity/Gravity_TieredClaims_Ledger.md` Staleness #10–#33**:  `ED_ORIENTATION.md`, `ED_MEMORY.md`, `ED_Zenodo_Uploads.md`, `ED_Outreach_Contacts.md`, `ED_Outreach_Drafts.md`, `PAPER_WRITING_CHECKLIST.md`, `CrankSafetyProtocol.md`, four `Memo_ED_*` audit memos, two `Update_Paper_*` notes, `Gravity_Quartet_TalkingPoints.md`, `ED_Public_Cosmology_Closure_Announcement.md`) is **working state, not corpus**, and is deliberately outside this index's scope.
