...
| SE-Koide | Koide's 2/3 as a Generation-Coherence Structure: an ED Re-description, a Toy Doublet-Grid Map, and a Neutrino Prediction (RE-DESCRIPTION + toy-model + consilience — NOT a derivation; mass sector inherited; obeys the B4 retrofit discipline; figures; referee-vetted). Companion scoping notes: `Note_MassArc_Koide_CoherenceReframe.md`, `Note_MassArc_BindingMassScale_Scoping.md`. | substrate-evaluation | WRITTEN (referee-vetted) | `physics-papers/substrate-evaluation/Paper_Koide_GenerationCoherenceMap.md` (+ `.pdf`) |
