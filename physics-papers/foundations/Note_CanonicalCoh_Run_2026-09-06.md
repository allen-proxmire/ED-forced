...
> **✅ CONFIRMED ON TEN SEEDS 2026-09-06 (#107) — and a separate problem found.** (C) is **0/10 crystal in every arm at every coupling**, `R ≈ 0.01–0.07`, not a near miss. **But against the `k_phase = 0` control (`ξ = 2.1`), only the `|acc|/n` arm RAISES correlation (`→ 4.1`); both canonical terms LOWER it** (`Grad` `→ 0.8`, canonical `Coh` `→ 0.7`), **reaching the paper's own “`ξ < 1`, no binding at all” failure.** **So Knots-safety is robust, and the attractive/constructive-sign claim holds only for the probe's own functional.** `foundations/Note_CondC_MultiSeed_2026-09-06.md`.
