...
- **Cheapest next check:** re-run `Paper_PhaseCoherence_P12Coh`'s probe with the phase term explicitly assigned to `Grad` rather than to the selection functional's `Coh`, and see whether the finite-reach result survives. **If it does, §5's open item is answered and the `cos Θ` discharge keeps both supports.**
