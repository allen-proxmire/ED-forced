...
>
> **And the extensive/intensive worry, re-framed the same day (#114).** The caveat above invites the question *“is `Σ_C`'s extensive `−Grad` right at all?”* — **that conflates two things.** **`Σ_C` is a landscape**, and its `Grad` term is the Dirichlet form summed over edges; nothing challenges that. **How a chain SELECTS among candidates using the landscape is a separate rule**, and the extensive/intensive difference lives entirely there — **so the functional can stay extensive while selection normalizes.** **And ED's own commitment rule is already normalized:** `Paper_003`'s **`P-LinRate`** gives `Pr(commit to K*) ∝ b_K*/Σ_K' b_K'` — **a probability, not a raw score.** **So intensive selection is the corpus-consistent reading and the simulator's argmax-over-raw-`Σ` is the outlier**, which is the strongest support (7) has: **the form that binds is the form ED's own selection postulate prescribes.** *(The gap, stated: `P-LinRate` normalizes over the **channel set at a locus**, the probe by the **coordination number of the growth front**; they coincide only if the candidate set is the neighbour set, which nothing states.)* `foundations/Note_ExtensiveGrad_Question_2026-09-06.md`.
>
