...
> **⚠ ANSWERED SAME DAY (#115) — THEY ARE NOT, and §5 above over-reached.** The probe's candidate set is **loci** (`compute_candidates` → `graph.admissible_neighbors`); `P-LinRate`'s is **channels**; and **canonical P03 names them as two separate index sets.** **They cannot be identified either:** `Paper_MS-I` derives `SU(N)` from channel multiplicity, so a channel–edge bijection would force `N = 1` per edge and **collapse the gauge group to `U(1)`.** **So the link is an ANALOGY between two normalized rules over different sets — suggestive, not derivational.** What survives is that **both rules are normalized**, leaving the simulator's argmax-over-raw-`Σ` an outlier against the corpus's only stated selection rule. **What does not survive is §5's claim that `P-LinRate` prescribes the probe's normalization.** `foundations/Note_NormalizationGap_2026-09-06.md`.
