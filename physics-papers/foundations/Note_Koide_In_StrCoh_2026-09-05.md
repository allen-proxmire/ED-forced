...
>
> **What it adds, flatly:** with `b = √m`, **`M_eff = 1/Q`** to `2.2e-16` — so `Q = 2/3` ⇺ `M_eff/N = 1/2`, the leptons at **exactly half-saturation of `M_eff ≤ N`**, the Cauchy–Schwarz bound proved independently the same evening. 
>
> **⚠ TIERED 2026-09-08 — and it does NOT converge with §1. See `Note_Koide_Tiering_2026-09-08.md`.** ~~*An earlier version of this block read: “That converges with §1 of this note from the other direction — `Str + Coh = (Σ√m)²`, so `Q = Str/(Str+Coh) = 1/M_eff` is the same algebra reached twice.”*~~ **Withdrawn. The second equality is false.** §1 sets `P_i = √m_i`, i.e. **`b = m`** — under which `Str + Coh = (Σ√m)²` and `Q = Str/(Str+Coh)` hold exactly (`0.66666051`), but **`M_eff = (Σm)²/(Σm²) = 1.119`, not `1/Q = 1.500`.** The probe sets **`b = √m`** — under which `M_eff = 1/Q` holds exactly, but `Str = Σb = 53.1 MeV` is no longer the sum of the masses, so `Str/(Str+Coh)` is no longer `Q`. **Both hold only if `m = 1`.** *Two different substitutions, each chosen so one ED quantity lands a known number — which is precisely the `Paper_030` caution §5 of this note raises against itself.* **Neither assignment is licensed anywhere in the corpus**, and `Paper_MassWithoutMass_BindingInertia` separates commitment rate from mass **by measurement**. **⚠ CORRECTED 2026-09-08 — see `Note_BAssignment_ForALepton_2026-09-08.md`. The “no map exists” half of this is FALSE and is withdrawn:** I searched `foundations/` and `substrate-evaluation/` and reported a corpus-wide negative. **The map is in `../event-density/arcs/`** — `arc-foundations/bandwidth_signature_construction.md` eq (4), `σ_τ = ℏ√(Σ_X w_τ^X ⟨(∂ln b^X)²⟩_τ)` with `m_τ = σ_τ/c²`, **form-FORCED, since Arc M.** **The conclusion nonetheless STRENGTHENS:** eq (4) is **exactly invariant under `b → λb`** by construction (the log-derivative is chosen to separate rule-type structure from chain-initial-condition amplitude), so **no pointwise `b = f(m)` can reproduce it** — verified, `σ` unchanged to all digits across `λ = 10⁻⁶ … 10³`. **`b = m` and `b = √m` are therefore not merely unlicensed but INCOMPATIBLE with a forced corpus result**, and `arc-M/mass_ratio_constraints.md` `R3-E` already **REFUTED** the `w`-pattern→ratio route that Koide would need. **The tierable content is an `Open` item: assign `b` for a charged lepton.**
>
> **And a SECOND, independent blocker applies, already in this folder:** `GENERATIONS_LICENCE_2026-09-07.md` establishes from `primitives/concepts/chain.md` §1 that **different rule-types are different KINDS OF CHAIN**, and arc Q §3.1 makes generation = rule-type — so **the three leptons are not three channels of one chain**, and no per-chain functional (`Str`, `Coh`, `M_eff`, `Σ_C`) ranges over them. *That note's own §4 already wrote “the `Σ_C = −½Str` opening — DEAD — `Σ_C` is per-chain and does not apply”; it applies to `M_eff = 1/Q` identically.* **This blocker is prior to the `b` question: a licensed `b` would not rescue either reading, because there is no sum to put it in.** **And it matters more than it did on 2026-09-05, because `M_eff` gained a governing postulate that evening (`P-Multiplicity-Extensive`, `q-compute/Paper_054` §3.1).**
>
...
>
> **And §4's verdict is unchanged.** Arc Q's *“ED forces exactly 3 generations — REFUTED”* stands, and neither ED nor Rousselle derives three (**he assumes `ℤ₃`**). *What is worth re-examining is arc M's stated REASON — “ED produces dichotomies, not continuous numerical relationships” — since `Q = 1/M_eff` at half-saturation of an integer bound would be a continuous number out of a counting structure. **The reason, not the verdict.*** **⚠ 2026-09-08: that re-examination is now suspended, not pending** — the `M_eff` route rests on the unlicensed `b = √m`, so it does not currently bear on arc M's reason either way.
