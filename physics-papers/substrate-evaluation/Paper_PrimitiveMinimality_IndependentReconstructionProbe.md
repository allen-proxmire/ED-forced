...
**A standing question left by the phase defect:** if P09 already carries a `U(1)`, how much work is the Solèr selection argument doing when it "selects" ℂ from `{ℝ, ℂ, ℍ}`? Two independent models converging on "phase must be injected" is not an answer, but it is a reason to look. **ANSWERED 2026-09-04** by an F2 audit of the ℂ-selection chain (`qm-kinematics/QMKinematics_TieredClaims_Ledger.md`, addendum; `gravity/Gravity_TieredClaims_Ledger.md` Staleness #21). The two halves of the selection differ. The ℍ-exclusion does real work and names its premise. The ℝ-exclusion does less than it appears: **P09 alone is consistent with ℝ**, since `U(1)` is an angle and real-Hilbert QM represents angles by rotations; what excludes ℝ is `Paper_001`'s choice to write that angle as a *complex scalar* `e^{iπ}` rather than a rotation `J(π)`. **So the blind runs were right that something must be injected** — ED's answer is P09 *plus* a representational commitment, and only the first is a declared primitive. Originally recorded as: **Open question, not a finding.**
