...
| **Both full seeded runs took the commitment rate linear in bandwidth — which *is* P-Commitment-Linear.** *Corrected 2026-09-04: an earlier row read this as the runs taking a branch ED rejects. Wrong. ED agrees with them on the linearity; the runs are convergent support for the postulate.* The divergence is one step later — both identified commitment rate **with** clock rate, while `GR-I` §4.2 puts the front on the null cone (`0 = −N² + b⁻¹b²`) to get `N² ~ b`, so the clock rate is `√b` while the commitment rate stays linear. That is what yields `g₀₀g_rr ~ −1`, the Einstein branch and the factor of two | **Measured** (n = 2), and it points two ways: convergent support for P-Commitment-Linear, and evidence that the **front-null identification** is the step no outsider makes. Rigor is owed to GR-I §4.2, not to the linearity |
