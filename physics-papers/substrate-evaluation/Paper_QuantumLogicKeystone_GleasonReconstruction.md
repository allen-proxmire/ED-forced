...
2. **The complex field is *selected*, not derived from nothing.** ℂ is chosen from Solèr's `{ℝ, ℂ, ℍ}` by two standard reconstruction arguments grounded in ED primitives **and in two named paper-specific premises**: the ℝ-exclusion rests on ED's interference being *continuous* in the relative phase (P-Quadratic-Strain / the Born-rule reading, §5 step 2), and the ℍ-exclusion on ED having a genuine `⊗` (P-Bipartite-Mapping / Paper_063). This is an account-tier selection, not a from-scratch derivation. *(Premises named 2026-09-04; the ℝ-argument previously rested on an unflagged representational choice.)*
...
- **Rules out ℝ.** *(Rewritten 2026-09-04; see the note after this list for what it replaces and why.)* The argument is *not* “a real space cannot represent a phase” — real-Hilbert-space QM represents `U(1)` perfectly well via an antisymmetric `J` with `J² = −I` (the Stueckelberg complex structure). The discriminator is **whether relative phase is *continuously* observable**, and ED's own results settle it:
  1. **P09** gives each channel a continuous `U(1)` phase `π_K`.
  2. **ED makes relative phase observable, continuously.** The per-channel strain carries `2√(b^{(a)}b^{(b)})\,\cos\Theta_{ab}` (`Paper_QuadraticStrain_v1` §3.2), and the double-slit account carries `ρ = b_A + b_B + 2√(b_Ab_B)\,\cos\Delta\pi` (`Paper_005.5` §3.3). The dependence on the relative phase is **continuous**, not two-valued.
  3. **A real amplitude space cannot reproduce that.** Superposing real amplitudes gives `(x_A + x_B)² = x_A² + x_B² + 2x_Ax_B` — there *is* a cross term, but its only freedom is a **sign**. Continuous `cosΘ` dependence requires each channel's amplitude space to be at least two real dimensions carrying a rotation action, which is a complex structure `J`.
  4. **A `J` commuting with all observables is ℂ** (Stueckelberg). So the effective field is complex.

  Hence `K ≠ ℝ`. **The ED-side premise is step 2** — that ED's interference is continuous in the relative phase — and it rests on the strain reading (P-Quadratic-Strain) and the Born-rule reading, not on the canonical thirteen alone. *Stated explicitly here to match the ℍ-bullet below, which names its own premise.*
...
> **What the ℝ-bullet replaced, and why — 2026-09-04.** It previously excluded ℝ on the ground that *“ED's phase is a **scalar** amplitude factor (Paper_001, `√b·e^{iπ}`)”*. That was circular at one remove: **P09 supplies an angle, not a complex number** — `U(1)` is definable as `ℝ/2πℤ` or `SO(2)` with no reference to ℂ — so writing it as `e^{iπ}` rather than as a rotation `J(π)` on a real 2-space is a *representational choice* made in `Paper_001`, and the choice was doing the excluding. **P09 alone is consistent with ℝ.**
>
> Two candidate repairs were tried. **P03/P07 fail**, and the reason is worth recording so it is not retried: real-Hilbert `J` acts *within* each channel's amplitude space and never mixes channels, so P07's intrinsic-identity commitment is not engaged, and P03 requires only that `J` be locus-independent. Neither primitive touches the question. **The continuity argument now in the bullet does work**, and it changes the character of the claim in two ways: it is non-circular (the premise is a corpus *result* with its own tier and falsifier, not a notation), and it is **empirically anchored** — were ED's interference sign-only rather than continuous in `Θ`, ℝ would be readmitted. That is a statement about the world; the notational version was not.
>
> **The tier is unchanged: account.** See `qm-kinematics/QMKinematics_TieredClaims_Ledger.md`, addendum 2026-09-04, and `gravity/Gravity_TieredClaims_Ledger.md` Staleness #21.
