...
- **Open:** a fully written-out `δQ=TdS` derivation of the *khronometric* field equations at a local horizon (**now done — `microscopic-completions/Paper_KhronometricEquationOfState_Jacobson`**, which confirms the §6 clean-equilibrium verdict explicitly; the closed-form aether-entropy leading power below remains the open sub-piece); a commitment-statistics, continuation-free derivation of the horizon `2π`/temperature (the one input still gotten via the reversible-time Euclidean route; `BH_Thermal2Pi` §4); the closed-form aether-entropy leading power that would harden §7 from inference to computation; the value of the grain / `λ_local` (inherited, as is Jacobson's `l_c`).
