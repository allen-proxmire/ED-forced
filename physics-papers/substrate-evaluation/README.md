...
- **Two honest edges to read carefully** (the map overrides the paper's framing):
  - **The Quantum-Logic Keystone (Gleason)** — the ℂ-field selection and Born non-contextuality are grounded, and (updated 2026-07-29) channel-orthogonality is **reduced to channel-distinctness *given the representation*** (operational distinguishability), with the equal-norm condition **discharged (2026-07-10)** — so the residual is narrowed to the **Solèr lattice technical rigor alone** (**"reduced to the Solèr residual"**), stronger than the earlier "three-routes-fail / blocking" verdict but still short of a closed uniqueness theorem. Read it as a *reconstruction with grounded ingredients and account-tier joints*, not a completed proof — and no longer a structural block.
...
| [How Our Best Theories Coarse-Grain Reality](Paper_HowTheoryCoarseGrainReality.md) | the coarse-graining ledger: each major theory = a blur of the one bandwidth field (kept column → its variables, discarded column → its symmetries); G and GR as seam results; Born/collapse as the un-blurred commitment; thermodynamics as the receipt; the live leaks (sub-`a₀`, `M_crit`) located | Synthesis (draft, referee-passed; per-entry tiers inside) |
| [Gravity as an Equation of State](Paper_GravityAsEquationOfState.md) | connects ED to Jacobson's 1995 "Einstein equation of state": ED grounds his three assumed inputs (severance = the causality barrier; grain = his fundamental cutoff; the severance count = the entropy density `η` he leaves *undetermined*, filling the "microscopic theory of spacetime structure" gap he flagged), lands the exact `G=c³ℓ_P²/ħ`, resolves the arrow paradox (arrow builds the horizons; GR = their equilibrium equation of state), and shows ED's luminal-cone khronometric departure is benign → clean equilibrium → full Einstein | Synthesis (draft, referee-passed; §7 co-suppression at inference tier) |
| [How Our Hardest PDEs Coarse-Grain Reality](Paper_HowPDECoarseGrainReality.md) | companion to the above, the two open-problem shadows: Yang–Mills (`F²` action derived-given-inputs; mass gap mechanism-tier, continuum survival open) and Navier–Stokes (fluid form forced, no-blow-up structural-positive on two declared postulates), then the finite-grain **singularity family** — the `P04+V5+P11` triple that obstructs NS blow-up is the same one that forbids the black-hole curvature singularity (`Paper_042`), with NLS/Keller–Segel/MCF/Ricci flagged same-shape and unexecuted. The triple-identity is stated at **synthesis tier**, not a theorem (see target #15) | Synthesis (draft, referee-passed; per-entry tiers inside) |
...
| Paper | Result | Tier |
|---|---|---|
| [The Quantum-Logic Keystone (Gleason)](Paper_QuantumLogicKeystone_GleasonReconstruction.md) | reconstructs the ℂ-field + Born non-contextuality; channel-orthogonality **reduced to channel-distinctness given the representation** (equal-norm discharged 2026-07-10) → residual = **Solèr lattice rigor alone** | **Reduced to the Solèr residual** (#8b; stronger than "blocking", short of a closed theorem) |
...
- **[Cold Models Cannot Build ED. They Can Audit It. The Asymmetry Is Structural.](Note_ColdModelsCannotBuildED_ButTheyCanAuditIt.md)** — the **verdict** drawn from eleven runs across two model families in three conditions (blind construction, seeded construction, adversarial audit). Not a physics result. Seeded runs given the thirteen primitives *and* V1/V5 *and* named targets all reach Newtonian scalar gravity and stop where `Paper_GR-I` begins; three specific steps go unfound, including the front-null identification that puts ED on the Einstein branch. The same families audit well. **The asymmetry is structural — auditing has a local oracle and construction does not — so it is not a prompting problem.** Records that the one genuinely new result of that day came from reading the corpus, not from any model.

- **[What Independent Models Add When Given ED's Primitives and Nothing Else](Paper_PrimitiveMinimality_IndependentReconstructionProbe.md)** — five runs, two model families, three conditions: blind (ontology sketch only), seeded with P08 swapped out, and seeded with the full primitives plus V1/V5. Not a physics result: a measurement of **where outside readers must add something, and what they postulate that ED derives**. Three findings. (1) Unforced additions land in the **coarse-graining layer**, which is not among the thirteen — ED discloses it as per-paper regime language with `MetricFromTheGraph` F5 as its falsifier. (2) Both full runs **postulate the `1/R` profile** that `Paper_026`/`Paper_027` derive from the channel count, and both identify commitment rate **with** clock rate, where `GR-I` §4.2 puts the front on the null cone and converts a linear commitment rate into a `√b` lapse — the step that gives `g₀₀g_rr = −1` and the factor of two. Note the runs *agree* with ED's P-Commitment-Linear; what no outsider found is the front-null identification. (3) Removing P08 was actually tested and **costs the inverse-square law and `G`**, so the coarse-graining addition cannot be paid for by deleting it. Also records that ED's isotropy position is *stronger* than the runs demanded, since P4 measures the washout on a genuine cubic lattice rather than assuming randomness.
