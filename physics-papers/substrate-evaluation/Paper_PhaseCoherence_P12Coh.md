...
- **P12-Coh is operationalized** as the phase-coherence content, and the **quadratic strain reading** of the Interference-Gravity paper is grounded. *(✅ **Corrected and SETTLED 2026-09-05.** This line originally read “$\mathrm{Coh}$ read as $|\sum P|^2$”, **the full square** — contradicting §2's definition as the **interference part only**, and colliding with `P-Quadratic-Strain`, which assigns the full square to `Str`. **§2 is the correct one.** A three-way sign check against `Paper_030`'s `a = a_N + √(a_N a_0)` settles the split uniquely: **`Str` takes the diagonal, `Coh` takes the off-diagonal**, exactly as §2 has it. **So this paper's `Coh` needed no change; its summary line did.** And the result strengthens: because `Coh` enters `Σ_C` with a **plus** and the MOND correction is attractive, **`cos Θ_LH > 0` is now required by the assignment** — giving the constructive sign a second, structural support independent of this paper's measured one. `foundations/Note_SigmaC_SignCheck_2026-09-05.md`.)*
