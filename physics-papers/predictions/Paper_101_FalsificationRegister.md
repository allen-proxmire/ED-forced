...
| 2 | **Evolving MOND scale `a₀(z) = cH(z)/2π`** ⭐ (ML 1.15; Paper_038 CO-3 / Paper_031 §8.8) | the transition scale tracks the *live* Hubble rate — `α = 1` in `a₀ ∝ H^α`, a shape ED cannot refit (no second scale to bend the power) | MOND → `a₀` *constant*; ΛCDM → no such scale at all | MUSE-DARK III (A&A 2026) **now**; Rubin/Euclid next | **data-confronted:** ~30σ evolution detected — kills constant-`a₀` MOND, **ED's *direction* confirmed**; **recasting** the survey's points as a power law gives `α ≈ 1.18` (**an ED-side re-fit — the survey publishes a *linear* fit, not a power law**), putting ED's forced `α = 1` in mild tension — **2.3σ on a computed conversion budget (2026-09-07): `α = 1.15 ± 0.07`, the published `±0.04` omitting 73% of the variance; survey systematics on top and unquantified. **⚠ 2026-09-07: the ~30σ DETECTION is itself under a coherent systematics challenge (Rusishvili 2026, not peer-reviewed, effect sizes arithmetically checked) — if it holds, the win and the tension fall together and the status is UNTESTED**. Call won, exact rate an OPEN tension — **not a refutation** |
| 2b | **`H₀` from rotation curves: `H₀ = 2πa₀/c`** (ML 1.16; `Paper_ED_Pred_H0_From_a0.md`) | the MOND scale is horizon-set, so measuring `a₀` measures `H₀` — **conditional on the disputed `2π`**, hence a *test* of that coefficient, never support for it | MOND → `a₀` is a constant of nature, **no such probe exists**; ΛCDM → no such scale | Desmond 2023 **now** (`H₀ = 77.0 ± 6.3`, +0.62σ SH0ES / +1.50σ Planck); needs ~4% on `a₀` to arbitrate | **registered, not yet decisive** — refuted if `a₀` reaches ±0.05 and the CMB `H₀` holds (≈3σ) |
| 3 | **Merging-cluster offset–velocity KNEE** (OffsetVelocityLaw; ML 1.13) + **group-embedded DF2/DF4 knee** (ML 1.14) | sharp *step-like* knee: flat → knee → linear → ceiling; fast mergers show >2 lensing peaks | ΛCDM → velocity-*independent* scatter (no knee); MOND → *smooth* roll-off (no knee) | ceiling + >2-peak leg testable **now** on El Gordo-class mergers; full knee = Rubin/Euclid | **experiment-ready** (shape robust; knee *location* inherited) |
| 4 | **Class-A decoherence wall** (Paper_056; ML 4.1–4.2) | sharp architecture-*independent* mass wall — the **shape** is the claim, not the number; 140–250 kDa is a 2-point extrapolation, now under pressure (interference already shown at ≥170 kDa, Nature 2026, inside the window, still QM-consistent). The 3–6% second-harmonic-fringe leg is **CONFOUNDED (2026-07-24)**: KDTL grating optics make harmonics anyway; a real test needs *excess* over the Nimmrichter–Hornberger prediction from raw fringes | smooth, environment/architecture-dependent decoherence; no universal wall | matter-wave interferometry (Arndt/Vienna, LUMI); now ≥170 kDa | pre-registered; the **form** (sharp, architecture-independent) is the claim; the kDa value is anchored + under pressure; the 2nd-harmonic leg is confounded |
| 5 | **Class-C error-correction plateau** (Paper_058; ML 4.10) + **Class-B exponential rigidity** (4.9) | logical error floors at `Γ_plateau > 0`; Class-B suppression `∝ e^{−ξ/ξ₀}` | fault tolerance → error → 0 exponentially with code distance | surface-code / topological-qubit scaling — **industry is climbing code distance now** | near-term; a test arriving *for free* |
| 6 | **BTFR zero intrinsic scatter** (Paper_031; ML 1.4) | *exactly zero* intrinsic scatter in the deep-MOND asymptote (the slope-4 is a postdiction; the zero-scatter is the bet) | ΛCDM halo models → nonzero scatter from formation-history variance | SPARC public data, improving precision | consistent so far; **the cleanest single number to miss** |
| 7 | **Dark energy strict `w=−1, ẇ=0`** (ML 5.1) | saturation, not quintessence — `w` pinned to −1, no evolution | quintessence/phantom allow `w≠−1`, `w_a≠0` | DESI / Euclid `w₀–w_a` | **live** — if DESI evolution firms up, a near-term kill (a real risk = a real weapon) |
| 8 | **Non-thermal Hawking correction** (Paper_047; ML 2.1) + **Planck-mass remnant** (2.2) | sign-definite `n_H[1 − c₁(ω/ω_c)²]`; evaporation halts at `M_⋆ = M_P` | exact thermal spectrum; complete evaporation | analog gravity (BEC/EIT/optical) | near-term in labs (`c₁~O(1)` inherited; conditional on P-V5-Even) |
| 9 | **PPN `α₂ = 0` exactly** (GR-IV; ML 1.5) | `α₂` identically zero (both cones luminal); `α₁` nonzero but ≥70 orders below bound | generic khronometric / Lorentz-violating class → `α₂ ~ O(1)` (GR also = 0) | LLR, pulsar timing, `|α₂|<10⁻⁷` | inside bounds — a clean **kill-switch ED passes** (not a positive vs GR) |
| 10 | **Matter-sector prohibitions** (ML 3.1/3.2/3.5) | no parity-violating (chiral) `U(1)` force; no stable gauge group beyond `SU(3)`; no anyons in genuine 3+1D | general gauge theory allows all three | any counterexample = one-line kill | standing prohibitions; distinctive (competitors don't make these) |
| 11 | **FRAP `α_R = 1/6`** (ML 4.4) + **AFM dewetting invariant −1.30** (4.5) | porous-medium front `R∼t^{1/6}`, not Fickian `t^{1/2}`; motif-ratio median −1.30 | Fickian diffusion `t^{1/2}`; no invariant | benchtop, ~$500–1500, weeks | protocol-ready; forward tests of the (self-published, not peer-reviewed) UDM postdiction |

### 0.3 The postdiction column — consilience, NOT weapons (keep separate)
Reproductions of already-known values; supporting evidence, but they cannot end an argument and several are shared with ΛCDM/MOND/GR: **`a₀ = cH₀/2π`** (the *local* value is a ~10% postdiction, value known since 1983; but its *redshift evolution* `a₀(z)=cH(z)/2π` is a genuine forward weapon, now data-confronted at ~30σ — promoted to Tier-1 row 2, not a postdiction), **BTFR slope-4** (data `3.95±0.08` predates the paper), **Λ-smallness `Θ_ED≈10⁻¹²²`** (inherited + reframing; the naive first-principles attempt failed by ~60 orders), **`S=A/4`**, **`c_GW=c`** (structurally *inherited* — a survival, not a risked bet), **`n_T=−r/8`** (tests standard inflation), **spin-statistics `(−1)^{2s}`**, **stochastic GW `Ω∝f^{2/3}`** (self-labeled "model-degenerate postdiction"). Internal-consistency checks (e.g. the RLC-circuit isomorphism) are not empirical discriminators at all.

### 0.4 Genuinely confirmed so far (the floor)
- **Universal Degenerate-Mobility law** `D(c)=D₀(1−c/c_max)^β` — self-authored Zenodo preprint, May 2026 (not peer-reviewed); a *derived form* fit to existing published data across 10 materials, `R²=0.986–0.999`. A strong **postdiction** (data predate the derivation), the corpus's best data-match — but not peer-reviewed, not a forward test (the FRAP `α_R` test is the forward leg).
