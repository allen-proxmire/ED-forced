...
**This is the Mott–Ioffe–Regel limit.** MIR: resistivity saturates when `k_F ℓ ~ 1`, i.e. the mean free path reaches the lattice spacing.

> **⚠ CORRECTED 2026-09-05. This sentence previously continued:** *“In ED that lattice spacing **is** the effective transport cell `a_eff`, and the flooring is not an empirical rule of thumb but the P04 budget hitting its per-cell wall.”* **That identification is asserted, unargued, and it is the only thing carrying the mechanism.** The chain above it cites **P08** *“DCGT-coarse-grained to the transport scale”*, and **neither step survives inspection**: P08's floor sits at `ℓ_ED ≈ 1.6×10⁻³⁵ m`, twenty-five orders below anything MIR is about, and **coarse-graining a floor at `ℓ_ED` does not install a floor at the coarse-graining scale** — averaging removes structure below the cell, it does not erect a barrier at it. `Paper_073_DCGT` §3.1 further states that `R_cg` is *“chosen anywhere in the scale-separated window”* with results *“insensitive to the specific choice”*, **so a floor there would make a measurable quantity depend on a bookkeeping decision.** **Settled reading: `a_eff` is the material's own emergent transport cell, inherited, not derived.** ED does not explain MIR; it inherits it. What survives is the *sharing* claim. Full argument: `Note_WhatIsAEff_2026-09-05.md`; gravity ledger Staleness #66.

So the flooring at the cell is **taken from the material**, and the P04 budget's role is not to produce the floor but to say that **momentum and charge spend against it together** (§4). The quantum-simulation demonstration of collision-driven resistivity saturation in an optical lattice (the article that seeded this lead) is a clean instance: the lattice site is `a_eff`, and the saturation is the budget wall.
...
| ~~Ceiling = MIR (mean free path floored at `a_eff`)~~ | ~~**identification / consilience** with standard MIR~~ → **I (inherited)** *(re-tiered 2026-09-05)*. *“Identification / consilience”* read as partial credit for grounding MIR in P04. **ED grounds nothing here: the cell is the material's and the floor comes with it.** `Note_WhatIsAEff_2026-09-05.md` §5. |
| `a_eff` itself | **I (inherited)** — the material's emergent transport cell. **Not** the DCGT cell `R_cg` (chosen, results insensitive to it) and **not** `ℓ_ED` (25 orders smaller). *(Settled 2026-09-05.)* |
...
- **F-EXIST — ⚠ THE “CONSILIENCE PASS” IS WITHDRAWN 2026-09-05. This falsifier's condition is met in the literature.** *Charge transport that continues rising without any saturation, with the mean free path driven below `a_eff` under control of confounds — falsifies `P-Scattering-Budget-Saturation`.* **That is the documented behaviour of bad metals:** Gunnarsson, Calandra & Han (Rev. Mod. Phys. **75**, 1085) name high-T꜀ cuprates and alkali-doped fullerides as violating the Ioffe–Regel condition, with the apparent mean free path passing below the interatomic spacing and no saturation. **The original parenthetical —** *“MIR saturation is already observed, so this is a consilience pass”* **— checked the falsifier against only the confirming half of the literature, which is not a check.** **It forks, and both branches cost the arc its target regime.** **(A)** If the mean free path is a real length there, **the postulate is falsified in those materials.** **(B)** If past Ioffe–Regel the Boltzmann `ℓ` is not a physical length — the standard reading — then F-EXIST cannot fire, **but neither can this mechanism apply**, since *“the mean free path is floored at the cell”* is then not a statement about anything in that regime; the arc must retreat to materials that do saturate, where no electron viscosity is measured, **leaving F-COONSET without a venue.** **(B) is the more likely physics and the worse outcome**, being a loss of purchase rather than a refutation that could be argued with. `Note_WhatIsAEff_2026-09-05.md` §4.
