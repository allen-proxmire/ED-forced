...
- **The universal mobility law (085) is data-matched (a postdiction)** — the constitutive law `M ∝ (ρ_max − ρ)^β` matched across 11 materials (`R² = 0.986`–`0.999`) from existing published data; the empirical anchor of the sector (and the capacity ingredient of the UDM, target #3). *(Self-authored Zenodo preprint, **not** peer-reviewed; the FRAP forward test is not yet run — see `predictions/Paper_ED_FRAP_Template.md`.)* **⚠ 2026-09-08 — do NOT treat the exponent `β` as something ED is owed.** *A substrate `β` was measured this session and **retired as a discretisation artifact**, and the comparison to UDM's `β` is **withdrawn as ill-posed**: the substrate side is a hop-acceptance fraction, UDM's is a macroscopic transport exponent, and the lattice-gas **correlation factor** plus a tracer→continuum step sit between them.* **The `(ρ_max − ρ)` FORM is what ED speaks to; the exponent is not a target.** (`black-hole/Note_Gamma0_Derivation_2026-09-08.md`; Target #15.)
