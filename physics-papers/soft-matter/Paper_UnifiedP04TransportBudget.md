...
3. **The load-bearing identification is a posit, not a theorem.** That both momentum transport and charge transport spend the *same* Adjacency band (§2) is grounded in **the P05/P07/P10 channel classification (`P-Band-Partition`)** + P05 being the sole inter-locus transport, but it is a declared identification (`P-Adjacency-Transport-Shared`), scrutinizable, not closed. **→ RE-GROUNDED 2026-09-05 (Branch 3 adopted).** The Adjacency band is **not** a decomposition of P04's scalar; it is **the class of P05-transporting channels at a locus**, a classification of $\mathcal{K}(u)$ licensed by P07 + P10, with the band total following from P04 additivity **given `P-Band-Partition`** (disjointness + exhaustiveness). **This arc's own 2026-09-04 flag predicted exactly this outcome** — *“the shared-budget identification may well survive on P04 additivity plus P05-as-sole-transport without needing four *named* bands, which would be a re-grounding rather than a retraction”* — **and it is that re-grounding. `F-COONSET` keeps its referent.** `primitives/P04_bandwidth.md`; `foundations/Note_FourBand_Branch3_2026-09-05.md`; gravity ledger #93.
...
This is grounded (**the P05/P07/P10 channel classification, `P-Band-Partition`**, + P05 as sole transport) but declared, not proven — it is the load-bearing joint (Preamble 3), and the single most important thing to attack. *(Re-grounded 2026-09-05; the Adjacency band is the **class of P05-transporting channels**, not a sub-part of `b_K`. Preamble 3.)*
...
| Finite per-cell `b_adj^max` | **P (`P-Locus-Bandwidth-Bound`)** *(was “form-forced”, corrected 2026-09-05)* | **`P-Locus-Bandwidth-Bound`:** *a locus's total bandwidth `b(u)` is bounded above.* **Canonical P04 supplies non-negativity and additivity, NOT an upper bound**, so the previous justification (*“P04 finiteness”*) claimed something P04 does not say. Compare `entanglement/Paper_065_Monogamy`, which does this correctly: its finite budget is `P-V5-Budget` + `I-V5` (inherited from `Paper_090`) **+** P04 additivity, each named. **This arc was assuming P04 supplied a budget; it now names one.** `foundations/Note_FourBand_Branch3_2026-09-05.md` §3.4. |
