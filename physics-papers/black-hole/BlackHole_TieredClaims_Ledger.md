...
*Papers 039–052 all dated 2026-05-13; HorizonTilingThreeCounts, 044.5, 047.5, 052.5 are July 2026. Read 2026-07-29.*
*Spot-checked directly against the papers (5, scaled to 18-paper folder): (1) 043's 1/4 = INHERITED — confirmed (title, audit rows 9–10, §4, conclusion); (2) the README headline "S=A/4 fully structural, both ED-derived" vs 043 — confirmed a real tension, refined below; (3) 051's capstone "paradox not generated / info not lost" = A→position on P-Substrate-Unitarity — confirmed verbatim (audit rows 9–10); (4) 052.5 merger-lag = PROVISIONAL/M2, zero D rows — confirmed (audit row 14); (5) HorizonTiling "one fact, not novel" — confirmed, but the extractor's claim that its "§4 supersedes 043" is a **fabricated quote** (the word "superseded" is absent; corrected in Staleness #1).*
...
| Area-law **form** `S ∝ A` | 043 | **D-via-I** | conditional on **P-Horizon-Participation + P-Multiplicity-g**; coefficient inherited | current (the **form**; the coefficient is a separate matter — see Inherited + Staleness #1) |
...
| Bekenstein–Hawking **1/4** coefficient | 043 | **I** — via log-g matching (`log g = π`); "matching, not derivation" (§4, conclusion) | current — **the in-folder EDG status of the 1/4 is INHERITED** (Staleness #1) |
...
| Arrow-native (continuation-free) **2π** | 047 / relocated `BH_Thermal2Pi` | OPEN — the derived-2π route is Euclidean (reversible-time); an arrow-native 2π is not yet supplied |

### Staleness & README-refinements
1. **The 1/4, honestly (the folder's most consequential tiering point).** *In the EDG black-hole folder*, the Bekenstein–Hawking 1/4 is **INHERITED** — Paper_043 says so in its title, audit rows 9–10, §4, and conclusion (`log g = π` is "matching, not derivation"). A *separately-derived* 1/4 (thermodynamic: κ = 1/(2r_s) + a near-horizon Rindler 2π, first law integrating to `S = A/4`) exists in the memo **`BH_Thermal2Pi_FromNearHorizonRindler`, which was RELOCATED OUT of EDG** to `event-density/foundations/relocated_from_EDG_2026-07-09/` (README line 63). So the "derived 1/4" is **not in this public folder**, and it rests on the **Euclidean/Rindler 2π** whose arrow-native form is still OPEN (Open table, last row). **The README *headline* (line 6) and line 63 advertise "`S = A/4` is now fully structural… both ED-derived" — optimistic relative to what the folder contains** (043 + the README's own table row 42 keep "coefficient inherited"). *Correction to the extraction pass:* it attributed a direct quote to "HorizonTiling §4" saying 043 is "superseded"; the word "superseded" appears nowhere in HorizonTiling — that paper says only that the 1/4 is derived *separately* (in the relocated memo) and concerns the *tiling*, not the coefficient. The real status is the two-route / relocated-memo picture above, not an in-folder supersession.
