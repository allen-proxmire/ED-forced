# The Hawking 2π From ED's Own Geometry: the Entropy 1/4 Is Structurally Derived

**Series:** Black-Hole Arc (entropy-coefficient companion to Paper_043 and `Paper_HorizonTilingThreeCounts`).
**Status:** Structural derivation given the 13 ED primitives (per Paper_087). Derives the thermal `2π` in `T = κ/(2π)` — previously read as inherited — from ED's *own* near-horizon geometry, completing the Bekenstein–Hawking `1/4`. Ported into EDG 2026-07-29 from the foundations working note.
