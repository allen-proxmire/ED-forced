...
>
> **So the blocker is a PRIMITIVE-LEVEL DECISION — does bandwidth quantize? — and not a definition anyone can write.** **⚠ AND — checked 2026-09-07 — NOTHING CONSUMES `N_bw`.** Every mention of it in `physics-papers/` is one of today's own flags or a passing quotation of the `min`; **`Paper_056` never uses it**, and `Paper_055` uses it only for the taxonomy row whose provenance was already found empty. **The definitional gap is real and nothing depends on it — a one-line note, not a constitutional decision.** *And the bound that DOES carry load is the other one: `N_bw = B_max/b_min` needs an upper bound too, that upper bound is the singularity family's per-cell capacity, and it is **already named `P-Locus-Bandwidth-Bound`** and declared across four folders.* **The live question is `Target #15`'s `Γ₀(ρ) → 0 as ρ → ρ_max` derivation, not a fourteenth primitive.** See `Note_Nbw_HasNoConsumer_2026-09-07.md`. *That is the correction to this flag: it named the wrong kind of task.*
>
...
>
> **⚠ TESTED 2026-09-07 — the P13 route is DEAD, and so is P07.** The commitment reserve specifies **no minimum draw** (`P04_bandwidth.md` defines `b_com(C)` and sources it to P11 without one), and even granting a draw the bound **does not reach `M_eff`**: the reserve bounds commitment events over a chain's **history**, `M_eff` counts **simultaneous** uncommitted channels — *two index sets, the arc's recurring defect in a third costume.* **The coupling is also inverted:** drawing on the reserve monotonically REDUCES `M_eff` (`39.1 → … → 1.0`). **And P07 — the only remaining route not running through a length — asserts channel DISTINCTNESS, not finiteness, and is an anti-merging principle: it forbids the very identification that could have bounded the count.** *Every geometric discreteness ED has is an EXTENT, and `M_eff` is not one; that is why both routes failed at the same joint.* **`ℓ_ED`/P08 and `D=3+1`/P06 remain untried, and no route is shown impossible.** See the ledger, 2026-09-07 (10).
>
