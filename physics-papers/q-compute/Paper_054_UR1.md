...
| **Power-law form** $\Gamma_{\mathrm{commit}}(M_{\mathrm{eff}}) = \Gamma_0\,M_{\mathrm{eff}}^\gamma$ — **RE-TIERED 2026-09-07 `A→regime` → `Grounded`**: exponential and log are **excluded** by the counting of `Σ_C`'s own two sums (single vs double); the exact form is `∝ M²−M` with `γ` **two-valued** (`→2` coherent, `=1` decohered, since `E[Coh]=0` exactly for random phases). **Residual open piece: the `Σ_C → rate` map.** `theory/gamma_commit_form_probe.py` | **Grounded** *(was A→regime)* | §3.2 asserts the power-law form with exponent $\gamma$. P04 + P11 + V1 give existence of commit-rate but do not force a power-law functional form. *(Round-4 Rereading Test: was hidden in D, now A→regime — power-law-in-multiplicity is a regime assumption.)* |
...
>
> **THE REMAINING GAP, STATED PLAINLY, BECAUSE IT IS REAL.** All of the above pins the multiplicity dependence of **`Σ_C`**. This section needs the dependence of **`Γ_commit`**, and it asserts only that *“gradients in `Σ_C` drive commitment”*. **The map from `Σ_C` to a rate is not supplied by the corpus and is not supplied here.** If that map is monotone and scale-free the exponents carry through unchanged; if it saturates, they do not. **So the tier moves from `A→regime` (the whole functional form assumed, with exponential and log live) to `Grounded` (the form forced given one named, much narrower map), and the open piece is now that map.**
