...
| **Power-law form** $\Gamma_{\mathrm{commit}}(M_{\mathrm{eff}}) = \Gamma_0\,M_{\mathrm{eff}}^\gamma$ — **RE-TIERED 2026-09-07 `A→regime` → `Grounded`**: exponential and log are **excluded** by the counting of `Σ_C`'s own two sums (single vs double); the exact form is `∝ M²−M` with `γ` **two-valued** (`→2` coherent, `=1` decohered, since `E[Coh]=0` exactly for random phases). **Residual open pieces (2026-09-07, worked and NOT closed): (a) the `Σ_C → rate` map is CATEGORIALLY absent — P12 gives an acceleration, not a rate; (b) a Kramers-type map would reinstate an exponential in `M`, so the exclusion holds for `Σ_C` and not necessarily for `Γ_commit`; (c) `Coh` is the STABILISING term, so the sign of the dependence is in question; (d) `Σ_C` sums over channels at one locus while `M_eff` spans many.** `theory/gamma_commit_form_probe.py` | **Grounded** *(was A→regime)* | §3.2 asserts the power-law form with exponent $\gamma$. P04 + P11 + V1 give existence of commit-rate but do not force a power-law functional form. *(Round-4 Rereading Test: was hidden in D, now A→regime — power-law-in-multiplicity is a regime assumption.)* |
...
> **POSTULATE NAMED 2026-09-07 — `P-Multiplicity-Extensive`.** §3.1 defines `M_eff` over *“substrate channel set `K`”* **without qualifying the index set**, and the corpus's only canonical chain-channel set is **per-locus-per-time**. The aggregation from there to a whole multi-locus object is what `Paper_056`'s mass law silently supplies. It is now named:
>
...
>
> **Why it is needed, and why it was invisible.** `Paper_087` §P12's chain-level bridge (2026-09-05) carries the sums only as far as **`K(C,u,t)` — a chain's channels at one locus at one time** — and **nothing in the corpus aggregates that into a whole multi-locus object.** A 175 kDa molecule is not one chain at one locus, so `Paper_056`'s `M_eff(m) ≈ αβ(m/m_u)` **was that aggregation rule, assumed rather than named.** *Naming it follows the corpus's own precedent three times over this week — `P-Locus-Bandwidth-Bound`, `P-V5-Budget`, `P-Band-Partition` — each a silent assumption made explicit, with the result kept and tiered rather than retired.*
>
...
>
> **⚠ A superseded framing is corrected here.** An earlier flag at this location called this a **collision between two papers**, reading §3's heading — *“Rate Quantities at the Chain's Locus”* — as scoping `M_eff` per-locus. **The heading cannot carry that:** it says *“Rate Quantities”* and **`M_eff` is not a rate** (§§3.2–3.4 are the three `Γ`s; `M_eff` is their input), §3.1's index set is **unqualified**, and **§3.3 — inside the same section — sums over locus PAIRS `u_A, u_B`.** *So there was never a textual commitment to per-locus, and the honest state is an unstated index set plus a missing aggregation rule, not a contradiction between papers.*
...
~~**Honest framing (round-4 QC):** the **power-law functional form** $\propto M_{\mathrm{eff}}^\gamma$ is an **A→regime assumption**, not derived. Alternative functional forms (exponential, logarithmic, etc.) are substrate-consistent.~~

> **✅ SUPERSEDED 2026-09-07 — the alternatives are EXCLUDED, and the exponent is not free.** `theory/gamma_commit_form_probe.py`. **The mistake this section made was to invoke `Σ_C` and then posit the multiplicity dependence instead of reading it off `Σ_C`.** The 2026-09-05 three-way sign check (foundations #10) settled the two terms' form, and their *counting* is then fixed, not assumed:
>
> $$\mathrm{Str}_K = \sum_a b_K^{(a)} \quad (M \text{ terms}), \qquad \mathrm{Coh}_K = 2\!\!\sum_{a<b}\!\sqrt{b_K^{(a)}b_K^{(b)}}\cos\Theta_{ab} \quad (M(M-1)/2 \text{ pairs}).$$
>
> **A single sum and a double sum cannot scale alike.** Three consequences follow, all checked numerically rather than argued:
>
> 1. **Exponential and logarithmic forms are excluded.** A finite double sum over `M(M-1)/2` pairs is **polynomially bounded** — no exponential can arise from it; and a logarithm grows slower than `Str`, which P04 additivity makes **exactly linear** (measured slope `0.999`). Both alternatives needed a characteristic multiplicity `M₀`. **The only candidate the theory offers is `M_cap`, which is *defined* as where `Γ_commit` meets `Γ_indiv⁻¹` — so putting it inside `Γ_commit` is circular.** And `Γ₀ ~ c/ℓ_ED` is a *time* scale while `M_eff` is dimensionless, so it cannot supply one either.
> 2. **The exact dependence is a two-term polynomial, `∝ M² − M`, not a pure power law.** Its local logarithmic slope runs `3.00 → 2.33 → 2.14 → … → 2.00` over `M = 2 → 1024`, **never leaving `[2, 3]`**. *The single-exponent statement in this section is the large-`M` limit — good to a few percent by `M ~ 100`, poor at `M ~ 2–10`.*
> 3. **`γ` is TWO-VALUED and set by phase coherence, not fitted.** Coherent: the pair sum survives, `Coh ~ M²`, **`γ → 2`** (measured `2.12`). Decohered: writing `z = Σ_a√b_a e^{iφ_a}`, `Coh = |z|² − Σ_a b_a` and `E[|z|²] = Σ_a b_a` **exactly** for independent phases, so **`E[Coh] = 0`** — the pair term does not degrade, it **switches off**, leaving `Str` and **`γ = 1`**.
>
> **This bears directly on the wall's sharpness (`Paper_056`), and it improves the argument rather than merely supporting it.** Sharpness had been a property *assumed* of a single power law. It is instead a consequence of the exponent **changing across the transition**: `γ ≈ 2` while the superposition is coherent, `γ = 1` once it is not. **The two sides of the wall are governed by different exponents, and the crossing is one-way.**
>
> **THE REMAINING GAP, STATED PLAINLY, BECAUSE IT IS REAL.** All of the above pins the multiplicity dependence of **`Σ_C`**. This section needs the dependence of **`Γ_commit`**, and it asserts only that *“gradients in `Σ_C` drive commitment”*. **The map from `Σ_C` to a rate is not supplied by the corpus and is not supplied here.**
>
> **⚠ WORKED 2026-09-07, AND IT DID NOT CLOSE — the attempt narrowed the claim above rather than completing it. Four things, three against interest.**
>
> **(a) The gap is CATEGORIAL, not merely unstated.** `Paper_087` §P12's operational content is `a_C = −∇_adj Σ_C` — an **acceleration**. This section needs a **rate**. **P12 contains no rate at all**, so the map is *absent from the primitives*, not stated loosely. Getting a rate from a force needs either a noise law or a distance in adjacency space, and the corpus supplies neither.
>
> **(b) The obvious lead does not work.** P11's Lindblad face carries rates `γ_α`, which looked like a landscape-to-rate map in the same framework. It is not: `lindblad_extension.md` fixes `γ_α` as *“proportional to the bandwidth transfer rate from `b_K^env` to the specific environmental mode `α`”*, with values *“inherited at the apparatus-physics level, not at the primitive-structural level.”* **Those are environmental decoherence rates. `Γ_commit` is meant to be intrinsic (`Γ₀ ~ c/ℓ_ED`). Different quantity.**
>
> **(c) ⚠ THE EXCLUSION OF EXPONENTIALS ABOVE IS OVERSTATED, and this correction matters.** Point 1 shows **`Σ_C`** is polynomially bounded in `M`. **That does not exclude `Γ_commit` being exponential in `M`.** A Kramers/Arrhenius map `Γ ~ exp(−ΔΣ/b̄)` composed with `Σ ~ M²` gives **`exp(−M²)`** — an exponential in `M` from a polynomial landscape. **The residual as first written named only *saturating* maps and missed this entire family.** So read point 1 as: *the alternatives are excluded **for `Σ_C`**; whether they are excluded for `Γ_commit` depends on the missing map, and the most standard physical map would reinstate one.* *(Whether Arrhenius is even available is itself doubtful — it needs a noise/temperature scale, and ED has no thermal primitive — but “doubtful” is not “excluded”.)*
>
> **(d) A SIGN QUESTION THAT MAY OUTRANK THE FORM QUESTION.** `Coh` is the term that grows as `M²`, **and `Coh` is the *stabilising* term** — it enters `Σ_C` with a plus, which the 2026-09-05 sign check settled. **If commitment is escape from a stability landscape, more coherence should mean a LOWER commit rate — so `Γ_commit` would FALL with `M`, opposite to this section's assumed rising form.** Raised, not resolved: it turns on whether commitment is driven by the *level* or the *gradient*, and on the adjacency metric, none of which the corpus fixes. **But if it holds, the issue is not the exponent's value — it is the sign of the whole dependence, and `M_cap` would not exist as constructed.**
>
> **(e) AND AN INDEX-SET QUESTION of the kind `INDIVIDUATION_DISAMBIGUATION` exists for.** `Σ_C`'s `Coh`/`Str` sums run over **the channel set at ONE locus** locus** (`Paper_087` §P12's 2026-09-05 chain-level bridge). `M_eff` for a matter-wave platform is a **whole-molecule** quantity spanning **many loci**. **Whether these are the same `M` is established nowhere, and `Paper_056`'s mass-scaling assumes they are.**
>
> **NET, and the tier stands where it was put.** `Grounded` remains right, but for a narrower reason than first written: **the `Σ_C` result is solid and the transfer to `Γ_commit` is not made.** *The open piece did not shrink further — it split into three sharper ones: the missing rate law (a), its possible sign reversal (d), and the index-set match (e). That is still progress over “the functional form is assumed”, but it is not the closure the previous pass anticipated.*
