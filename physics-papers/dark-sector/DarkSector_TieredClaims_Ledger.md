...
| Claim | Paper | What's derived | Inherited / open | Status |
|---|---|---|---|---|
| `a₀ = cH₀/(2π)`, parameter-free ~10% match | 029 | the `2π` from azimuthal-Fourier normalization of the dipole projection | ~10% **postdiction**; a₀ value inherited via H₀ | current |
...
1. **a₀(z) first data — a live, favorable currency update worth propagating.** MUSE-DARK III (A&A 2026): the acceleration scale **evolves** across `0.33<z<1.44` at **~30σ**, `a₀` rising `1.2→~2.4 ×10⁻¹⁰ m/s²` by `z~1`, which **excludes constant a₀** and kills constant-scale MOND — ED called the *direction* right where MOND called it constant. The **exact rate** is in mild tension (data run faster than the forced `∝H(z)`). This is close to AP's north-star (a confirmed distinctive call), but the exact-rate tension means **do not over-bank it**. **Recommend propagating** the confirmation-plus-tension to: `predictions/` (the a₀(z) weapon + 22-Ways), `gravity/Paper_029`/`KM-I`, and the flagship Report (memory notes `REPORT_CHANGES C2` needs a refresh from this folder).
