...
> ## ⚠ Migrated 2026-09-04 — read this first
>
> **This paper is a BANKED NEGATIVE.** It attempts the substrate derivation of $\xi_{\mathrm{canonical}}(H_0)$ and **the derivation does not close.** Its own status line says so, and §§3.5/6 say so again. It is preserved because a failed attempt that is *findable* prevents the attempt being repeated — which had already nearly happened: `Paper_096` did not cite this paper until 2026-09-04, so anyone attacking “derive $\xi_{\mathrm{canonical}}$ from the substrate” from the canonical corpus would have redone it. **Do not read the §3 machinery as a derivation.**
>
> **Provenance.** Migrated from `event-density/papers/Generative Papers/`, a fork frozen 2026-07-05 (see its `SUPERSEDED.md`). It is the first of the six `ED-SC 4.x` papers to move; the series is **not** superseded by `Paper_SCBU` — `Paper_SCBU` is *upstream* and its falsifier **F4** calls itself *“the bridge to ED-SC 4.x”*. Gravity ledger Staleness #52–#53.
>
> **Three corrections applied on migration**, since this paper predates them:
>
> 1. **The `2\pi` is DISPUTED.** §§2.2/4 cite `Paper_029`'s $a_0 = cH_0/(2\pi)$ and its *“azimuthal-Fourier signature”*. **That signature does not survive:** `Paper_028` §6.3 and `Paper_029` §5.1 both evaluate the azimuthal integral to **1**, so the factor cancels in their own displayed algebra and is reinstated by a normalization fixed nowhere. Re-tiered **Postulated / disputed** (gravity Staleness #10). **The scale $a_0 \sim cH_0$ is unaffected** and is now reached four independent ways. Any step here that leans on the `2\pi` *as a signature* is leaning on a disputed factor.
> 2. **$\xi_{\mathrm{canonical}} = 1.7575$ lu is quoted to unsupported precision.** The measurement artefact `ed-lab/outputs/ed_sc_3_1/xi_canonical.json` reports `xi_mean = 1.7575…` **with `xi_std = 0.303`** over ten seeds — a 17% spread, per-seed range 1.623–2.191. **The honest value is $\xi \approx 1.8 \pm 0.3$**, and it is processing-dependent: the same artefact's smoothed autocorrelation gives $3.05 \pm 0.53$, a factor 1.74 away. `Paper_096` §7 carries the full note. **This sharpens rather than weakens this paper's conclusion** — a target value with 17% scatter is a harder thing to derive, not an easier one.
> 3. **§6 is titled “Rewrite Note”.** Per `internal notes/PAPER_WRITING_CHECKLIST.md` failure mode 5, §6 should be a **Position Statement**. Left as-is to keep the migrated text faithful; flagged rather than silently retitled.
>
> **Not audited.** No claim-strength pass has been run on this paper against this corpus's standard (checklist §9 items 19–21). Its M3 self-verdict is its own.
...
- **I-029:** Transition acceleration $a_0 = c H_0/(2\pi)$ — supplies the azimuthal-Fourier $2\pi$ signature. **⚠ The `2\pi` and its azimuthal-Fourier reading are DISPUTED as of 2026-09-04 — the factor cancels in `Paper_028` §6.3 / `Paper_029` §5.1's own algebra. The scale $a_0 \sim cH_0$ stands; the coefficient does not. See the banner.**
- **I-091:** Kernel cascade across scales (V1 → V5 → effective-hydrodynamic).
- **I-096:** Cross-scale invariance at canonical operating point $\xi_{\mathrm{canonical}}$ (canon-internal value 1.7575 lu). **⚠ Measured value is $1.76 \pm 0.30$ lu (`σ = 0.303`, ten seeds; smoothed variant $3.05 \pm 0.53$). Five significant figures are unsupported — see `Paper_096` §7 and the banner.**
