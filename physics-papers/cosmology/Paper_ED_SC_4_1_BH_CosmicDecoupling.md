...
>
> **Arc caveat — the two-horizon identification.** This paper's `r_H` ↔ `R_H` projection sits alongside `Paper_047_5`'s M3 result that BH, Rindler, cosmological and acoustic horizons are one V5-saturated substrate object on four parameter axes, and alongside `Note_TheSwitchIsDerived_TheShapeIsInherited.md` (2026-09-04), which derives `Γ_cross` collapsing at `min(R_a, R_H)` from `Paper_039` §3.1. **Neither is cited here**, and both postdate it. A reader should treat this paper's horizon-anchoring as the earlier, coarser statement of a relation the gravity arc has since sharpened.
>
> **`ξ_canonical = 1.7575` lu is quoted to unsupported precision.** The artefact `ed-lab/outputs/ed_sc_3_1/xi_canonical.json` reports `xi_mean = 1.7575…` **with `xi_std = 0.303`** over ten seeds (range 1.623–2.191), and the smoothed variant gives `3.05 ± 0.53`. **The honest value is `ξ ≈ 1.8 ± 0.3`**, processing-dependent. `Paper_096` §7 carries the full note.
>
> **Provenance.** Migrated from `event-density/papers/Generative Papers/`, a fork frozen 2026-07-05 (see its `SUPERSEDED.md`). The `ED-SC 4.x` series is **not** superseded by `Paper_SCBU` — `Paper_SCBU` is *upstream*, and its falsifier **F4** calls itself *“the bridge to ED-SC 4.x”*. The six are kept together here as one series because `Paper_ED_SC_4_6` unifies them; each is cross-referenced from its own arc. Gravity ledger Staleness #52–#54.
>
> **The `2π` is DISPUTED.** Where this paper inherits `Paper_029`'s `a_0 = cH_0/(2\pi)` or its *“azimuthal-Fourier signature”*, the **coefficient** does not survive: `Paper_028` §6.3 and `Paper_029` §5.1 both evaluate the azimuthal integral to **1**, so the factor cancels in their own displayed algebra and is reinstated by a normalization fixed nowhere. Re-tiered **Postulated/disputed** (gravity Staleness #10). **The scale `a_0 ~ cH_0` is unaffected** and is now reached four independent ways.
>
> **§6 is titled “Rewrite Note”** where `internal notes/PAPER_WRITING_CHECKLIST.md` failure mode 5 wants *“Position Statement”*. Left faithful to the migrated text and flagged rather than silently retitled.
>
> **Not audited.** No claim-strength pass (checklist §9 items 19–21) has been run on this paper against this corpus's standard. Its **M3** self-verdict is its own.
