...
- **Paper #3 (Inner Product + Tsirelson):** four-band orthogonality + sesquilinear inner product structure that supports the participation measure. **⚠ ~~ARCHIVED-SOURCE CITATION~~ — flag raised and WITHDRAWN 2026-09-04.** **✅ FLAG WITHDRAWN, same day — the citation is SOUND and my earlier flag was wrong (Staleness #57).** *“Paper #3”* is a **Forcing Paper**, not an M-paper. `ARCHIVED_M_SERIES_NOTICE.md` archives the M-series (`paper_M0`–`M4`, omnibus, `NPP_M1`) and explicitly keeps ***“The Forcing Papers (#1–#19)”*** as the program's current vehicle. And `paper_M2`'s own abstract states that ED *“**is committed at the primitive level** to a four-band partition of bandwidth (P04 §1.5)”* — the partition is M2's **subject**, pre-existing it; what M2 tried and failed to do was show it **FORCED** from a deeper layer, and **that forcing attempt is what was archived**. The four-band partition is canonical primitive-level content. **Nothing here needs changing.** *The superseded flag is kept below as the audit trail.* **Original flag:** *“Paper #3 — four-band orthogonality”* is the **archived M-series** citation that `Foundations_TieredClaims_Ledger.md` staleness #2 identified on 2026-07-29 and **repointed in `Paper_089`** — *“canonical `Paper_087` has no four-band”*. **The identical citation survives here and was never fixed.** Canonical `Paper_087` defines P04 as *“bandwidth as non-negative additive scalar”* with no band partition. **Not resolved here:** whether P04 genuinely carries a four-band partition is a primitive-definition question, and `primitives/P04_bandwidth.md` asserts it in its own title while 087 does not. See gravity ledger Staleness #56.**
> **What this paper actually uses from Paper #3, checked 2026-09-05.** The phrase *“four-band orthogonality”* above is why this paper was carried as a **four-band dependent**. It is not one. **§2.5's audit table traces every load-bearing step and no row cites the partition** — the four-band phrase appears only in this dependency prose and never reaches the derivation. **The structural reason is stronger than the absence:** this paper's only use of bandwidth is the normalized sum $\sum_{K'} b_{K'}$ (in `P-LinRate` and again in the frequency limit), and **any partition of the channel set leaves that sum unchanged.** So the Born-rule derivation is **insensitive** to the four-band question — it cannot need the partition, and it is not evidence for it either. **What it does need** is P07 channel structure, P04 additivity over the channel set (disjoint channels, i.e. P04 exactly as canonically stated), **channel orthogonality** — which is what *“four-band orthogonality”* was reaching for, the relevant fact being that distinct channels are orthogonal so $\lVert\Psi\rVert^2=\sum_K b_K$ has no cross term (`Paper_007` §3.2), tracked as `P-Channel-Orthogonality` and unrelated to any band partition — and `P-LinRate`. `foundations/Note_Paper003_FourBand_Check_2026-09-05.md`; gravity ledger #91.
