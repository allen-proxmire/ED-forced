...
**APPLIED 2026-09-04.** The four steps are folded into §5's ℝ-bullet; the appeal to `Paper_001`'s carrier form is gone. The bullet names its premise (step 2) as the ℍ-bullet already did, preamble item 2 names both, and a provenance note beneath the list records what was replaced, why it was circular, and that P03/P07 were tried and fail. **Tier unchanged: account** — what changed is that the premise is now a corpus result with its own falsifier rather than a notation, and the claim is empirically anchored: sign-only interference would readmit ℝ.
