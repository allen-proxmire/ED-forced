...
- **Parsimony.** ED reduces the number of independent fundamental constants by one — Newton's $G$ is now downstream content rather than an independent input.
  ...
  >
  > **F-P run 2026-09-04, and the result narrows this claim.** The three companion roles were checked for how each fixes $\ell_{\mathrm{ED}}$. **None of them determines it independently.** `Paper_025` preamble item 2 and audit row: *"not derived — empirically identified with $\ell_P$ via Newton-recovery (Paper_027 §5.3); value-layer empirical content"*, labelled **I**. `Paper_111` items 3 and §1: *"the cutoff scale $\omega_c = c/\ell_P$ is INHERITED via Planck-scale matching."* `Paper_043` takes P08 as "Planck length" and needs its own postulate (P-Horizon-Participation) to fix the horizon count. **All three inherit the value from §5.3, which fixes it by matching $G$.**
  >
  > **So the paragraph above overstated the case, and the correction matters.** "Spent in several independent places" is true of the *uses* and false of the *determinations*. This is **one determination and three uses**, with no independent confirmation from any use. The parsimony is therefore an accounting claim about the **input list** — a rival framework needs $G$ *and* a horizon-count grain *and* a UV cutoff, where ED needs one scale — and **not** a claim of predictive success. Stated that way it survives; stated the other way it does not.
  >
  > **F-P as originally written cannot fire**, since a disagreement between roles requires two independent determinations and there is only one. It is reformulated:
  >
  > **F-P (revised).** *If any companion role ever determines $\ell_{\mathrm{ED}}$ independently of $G$ — a substrate derivation of the black-hole entropy coefficient that does not assume P-Horizon-Participation, or any measurement bearing on the UV cutoff — and the value disagrees with $\sqrt{\hbar G/c^3}$, the reduction is spurious and the corpus is carrying several scales under one name.* Conditional on an independent determination existing; none does today.
  >
  > **One quasi-independent comparison does exist, and it is off by a factor of 4.** `Paper_025` §6 compares its bound $N \leq 4\pi R^2/\ell_{\mathrm{ED}}^2$ with Bekenstein's $S \leq A/(4\ell_P^2)$ and reports that, with $\ell_{\mathrm{ED}} = \ell_P$, the two have identical functional form *"up to the dimensionless coefficient (1 vs. 1/4 — order-unity)."* That is the only place where the shared value is checked against an outside result rather than simply used. **It agrees in form and misses the coefficient by 4.** Recorded rather than dismissed: in this arc the forms have been reliable and the coefficients have not.
...
- **Paper #3 (inner product + orthogonal bands):** four-band orthogonality (P04 §1.5) supplies the strain-band content within the participation-measure decomposition. **⚠ ~~ARCHIVED-SOURCE CITATION~~ — flag raised and WITHDRAWN 2026-09-04.** **✅ FLAG WITHDRAWN, same day — the citation is SOUND and my earlier flag was wrong (Staleness #57).** *“Paper #3”* is a **Forcing Paper**, not an M-paper. `ARCHIVED_M_SERIES_NOTICE.md` archives the M-series (`paper_M0`–`M4`, omnibus, `NPP_M1`) and explicitly keeps ***“The Forcing Papers (#1–#19)”*** as the program's current vehicle. And `paper_M2`'s own abstract states that ED *“**is committed at the primitive level** to a four-band partition of bandwidth (P04 §1.5)”* — the partition is M2's **subject**, pre-existing it; what M2 tried and failed to do was show it **FORCED** from a deeper layer, and **that forcing attempt is what was archived**. The four-band partition is canonical primitive-level content. **Nothing here needs changing.** *The superseded flag is kept below as the audit trail.* **Original flag:** *“Paper #3 — four-band orthogonality”* is the **archived M-series** citation that `Foundations_TieredClaims_Ledger.md` staleness #2 identified on 2026-07-29 and **repointed in `Paper_089`** — *“canonical `Paper_087` has no four-band”*. **The identical citation survives here and was never fixed.** Canonical `Paper_087` defines P04 as *“bandwidth as non-negative additive scalar”* with no band partition. **Not resolved here:** whether P04 genuinely carries a four-band partition is a primitive-definition question, and `primitives/P04_bandwidth.md` asserts it in its own title while 087 does not. See gravity ledger Staleness #56.**
