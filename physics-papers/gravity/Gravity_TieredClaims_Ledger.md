...
*~30 papers. Read 2026-07-29 (extraction agent, then spot-checked).*
...
1. **a₀(z) + 2026 MUSE-DARK III data — the propagation gap (the big one, actionable).** Only **Paper_031 §8.8** reflects the 2026 confirmation (~30σ evolution, excluding constant a₀; ~8% local match; exact rate on the edge of / mildly faster than H(z); **α=1 is the decisive open number**). **Stale / predate it:** **Paper_029** (states the *static* reading, "a₀ tracks whichever H₀ is correct," no z-evolution — verified §8.5); **Paper_037** (a₀ RG-invariant, no a₀(z); not contradictory but uncited); **Paper_038 CO-3** (has the `a₀∝H(z)` prediction but is **not** data-confronted); **Predictions_and_Falsifiers** (omits a₀(z) as a live test entirely — lists only the static Milgrom coincidence; the flagship evolving-a₀ bet + its 2026 data are **absent from the falsification map**); KM-I / KM-II / One-Field / PhilPapers (use a₀=cH₀/2π without the evolution confrontation). **Recommend propagating** 031 §8.8's wording (α=1, ~8% match, faster-than-H(z) mild tension) into **029, 038 CO-3, and especially Predictions_and_Falsifiers** — this is also flagged in the dark-sector ledger (same weapon). *Do not over-bank: direction confirmed, exact rate under tension.*
