...
>
> **RESOLVED 2026-09-04 — branch chosen, and the reason §6.2 gives is not the right one.**
>
> **§6.2's conclusion stands; its argument does not.** `Σ₀ = −a₀R` is *not* an isotropic field, so "there is no static, source-independent, **isotropic** field of strength `a₀`" misses. A potential linear in `R` with constant gradient `a₀` directed along the chain–mass axis is exactly what a dipole aligned with the acceleration axis produces, and `Paper_030` §3.2 integrates along that axis. Anisotropy does not disqualify it. `Paper_030` line 123 also states the object's provenance explicitly — *"not a separately-postulated stability-landscape function; it is the coarse-grained potential **derived from Paper_029's dipole-projection mechanism via radial integration**"* — which §6.2 does not engage.
>
> **The real defect is one word, and it is in `Paper_030`.** §3.2 obtains `Σ₀` by integrating *"the **constant** cosmic-anisotropy acceleration `a₀`"* along `R`. But `Paper_029` §3.3 and §5.1 make the horizon content **first order in the chain's own acceleration**, `ρ_cosmic(θ) ∝ ρ₀(|a|/c)\cosθ`, and **zero for a non-accelerating chain**. Integrating it as a constant converts a **response** into a **source**. That is the step that should not have been taken, and it is upstream of this paper.
>
> **A third branch was attempted on that basis and it fails.** Restoring the dependence gives `b^{(H)} ∝ H₀|a|`, which would have been free: the diagonal term stays in the expansion, so the algebra is honest, and it vanishes as `a → 0`, so no constant-`a₀` floor survives and BTFR is safe without deleting anything. But the cross-term then reads `√(b^{(L)}b^{(H)}) ∝ √(GM H₀|a|/R)`, and deep-MOND self-consistency `a ≈ a_{cross}` gives `a² ∝ GM H₀|a|/R²`, hence **`a ∝ GM H₀/R²`** — a Newtonian-like `1/R²` scaling, not `√(a_N a₀)`. *The naive restoration breaks MOND.* Recorded as a negative.
>
> **Branch taken: (i).** `b^{(H)}` is treated as isotropic and R-linear, as §2 and §5.1 already use it; the diagonal term **stays** in the quadratic expansion, since it is generated by this paper's own postulate and cannot be dropped selectively; and `Paper_030` §5.3's regime assumption is **restored** to suppress it. §8's "two postulates to one" becomes **one and a half**: the P14 discharge (§5) stands and is the strong half; the regime-assumption discharge (§6.3) does not.
>
> **What this buys, and it is worth more than the discharge it costs.** The formal inconsistency is now a specific physical question: **`Paper_029` gives a horizon response first-order in `|a|`, and MOND requires a threshold at `a₀`. How does a linear response produce a threshold?** Neither paper asks this. It is arguably the deepest open question in ED's MOND account, and it was invisible while `Σ₀` was carried as a constant source. See `Gravity_TieredClaims_Ledger.md` Staleness #13.
>
> Recorded as Staleness #13 in `Gravity_TieredClaims_Ledger.md`. §6.3's "no regime assumption is needed" is **withdrawn**; §8's "two postulates to one" reads **one and a half**. §5 and the P14 discharge are unaffected.
...
- The per-channel bandwidth-as-potential conventions $b^{(L)}\propto GM/R$, $b^{(H)}\propto a_0 R$ (Paper_030 §4.2). **[FLAGGED 2026-09-04:** these are the *post-cancellation* forms. `Paper_030` §4.2 defines the same quantities *with* channel-count factors, $b_K^{\mathrm{loc}}\propto GM/(R\,N(R))$ and $b_K^{\mathrm{horizon}}\propto a_0R/N_{\mathrm{horizon}}$, and drops both in its next equation. This paper's own §4 uses the *with*-$N(R)$ form for the same symbol $b_K^{(L)}$, so both forms appear here, differing by $N(R)\approx10^{111}$ at galactic radii. **Settled 2026-09-04:** the cancellation is real (`Paper_027` §4.4's mechanism) and both forms are correct at their own stage — §4's is per-channel *before* summation, §2/§5.1's is *after*. The residual issue is that `Paper_030` §4.2 never shows the step, and that it silently requires $N_{\mathrm{horizon}} = N(R)$. See `Gravity_TieredClaims_Ledger.md` Staleness #16. Found by external audit; the in-house symbol sweep missed it because the two forms carry different names.**]**
...
> **✅ SETTLED 2026-09-05 — the quadratic form splits across TWO P12 terms, and the statement above needs amending accordingly.** As written this postulate puts the whole of `|Σ_a P_K^{(a)}|²` into `Str_K`. **The same object is also assigned to `Coh` by `Paper_PhaseCoherence_P12Coh`, and the two enter `Σ_C = Coh − Str − Grad` with opposite signs — so on the text as it stood, the cross term cancelled and this arc's MOND mechanism vanished.** A three-way check against `Paper_030`'s established `a = a_N + √(a_N a_0)` resolves it uniquely:
> $$\mathrm{Str}_K=\sum_a b_K^{(a)}\ \ \text{(the diagonal)},\qquad \mathrm{Coh}_K=2\!\!\sum_{a<b}\!\sqrt{b_K^{(a)}b_K^{(b)}}\cos\Theta_{ab}\ \ \text{(the off-diagonal)}.$$
> **Why no single `Str` can work:** the Newtonian piece goes as `1/R` and the MOND piece as `\ln R`, and `d(1/R)/dR < 0` while `d(\ln R)/dR > 0` — **so making both accelerations attractive requires the two pieces to enter `Σ_C` with opposite signs**, which one squared modulus cannot supply. **`Σ_C = Coh − Str − Grad` is already exactly that split: P12 was right and needs no amendment.** **Independently confirmed by P04:** bandwidth is non-negative, so §4's *“$\Phi_N=\sum_K b_K^{(L)}=-GM/R$”* cannot hold — and under this assignment it becomes `Φ_N = −Str`, the minus being P12's own. **Nothing in this paper's results changes**; only the destination of the off-diagonal moves. *(The postulate's name is now slightly off — the quadratic form lives in two terms, one of which is not strain — but it is cited widely and is not worth renaming.)* Full check and script: `foundations/Note_SigmaC_SignCheck_2026-09-05.md`; `internal notes/_check_sigma_sign.py`.
...
| 7 | $a_0$ is a dipole, not a monopole; $\Sigma_0=-a_0R$ removed | **WITHDRAWN 2026-09-04** (was D) | §6.2's reason (isotropy) is wrong; the diagonal stays. The real defect is `Paper_030` §3.2 integrating a *constant* $a_0$ where `Paper_029` makes the response first-order in $|a|$ — see the banner |
...
> **Sign corrected 2026-09-05.** This line previously read $\Phi_N=+\sum_K b_K^{(L)}=-GM/R$, **equating a sum of non-negative bandwidths (P04) with a negative number.** The leading minus is **P12's own `−Str`**: `Φ_N = −Str_K` summed over channels. **Absorbing that minus into `Φ_N` is what concealed the `Coh`/`Str` collision** settled above. No numerical result changes.
...
> **The sign transfers; the finite-reach measurement belongs to `Grad` — noted 2026-09-06 (gravity ledger #112).** `Paper_PhaseCoherence_P12Coh`'s build implemented `|acc|/n`, which is **`Grad`'s phase half intensively scored**, not the canonical `Coh` its §1.3 states. **What this result cites it for — the attractive/constructive sign — is unaffected**, because `+Coh` and `−Grad` both reward alignment and differ only by a neighbour–neighbour term. **What does not carry over is the finite-reach `ξ` measurement**, which is a property of `Grad`-intensive dynamics; canonical `Coh` shows no alignment gain under any normalization tried. **No claim here changes.** `foundations/Note_RepairTest_2026-09-06.md`.
