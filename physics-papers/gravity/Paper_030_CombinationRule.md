...
whose negative gradient yields the multiplicative geometric-mean combination $a = \sqrt{a_N \cdot a_0}$. The total substrate-level acceleration is $a = a_N + \sqrt{a_N a_0}$, which bridges the Newtonian regime ($a \to a_N$ when $a_N \gg a_0$) and the deep-MOND regime ($a \to \sqrt{a_N a_0}$ when $a_N \ll a_0$) smoothly and with no fitted parameter, **both limits following as limits rather than as regime assumptions.** *(Corrected 2026-09-04: this construction previously carried a third, standalone $a_0$ term from a gradient of $\Sigma_0$, which dominated the deep field and contradicted §3.4's own refutation of the additive combination; $\Sigma_0$ is now used only as the content normalization it is. See §3.2 and `Gravity_TieredClaims_Ledger.md` Staleness #35–#37.)* **Scope:** the profile is the deep-field law and is **not** licensed at solar-system accelerations — it saturates as $1-\mu \sim x^{-1/2}$ and is excluded there by ephemerides (§7.2); the standing crossover treatment is `Paper_KM-I` §6.3. **The geometric-mean form is the structural consequence of P14, not derived from a uniqueness-of-substrate-mechanisms argument.** The paper makes no claim that $H_0$ or cosmology is derived, no claim that MOND-as-phenomenology is correct as a complete theory, no claim that the ECR's specific transition profile matches every MOND interpolation function, and no claim that the rule is forced from nothing. The empirical case rests on cross-domain reach: Papers_027 + 029 + 030 (this paper) + 031 constitute the substrate-gravity arc, with BTFR slope-4 + zero intrinsic scatter (Paper_031) as the testable consequence.
...
> **RESOLUTION APPLIED 2026-09-04 — `a₀` is a normalization, and the step above used it as a force.** *(Recorded first as a proposal; applied the same day on instruction. The rewritten treatment is immediately below; §5.3, §6.3 and §7.1 are corrected to match.)* `Paper_029` §5.1 carries two separable objects: the **normalization** $\rho_0 = cH_0$ (no $|\vec a|$-dependence) and the **response amplitude** $(|\vec a|/c)\cos\theta$ (first order, zero at $a=0$). $a_0 = cH_0/(2\pi)$ is the *projected normalization*, and `Paper_029` §4.4 characterizes it in its own words as *“the **transition acceleration** — the **threshold**”* whose role is set by *“the **ratio** of $a_N$ to $a_0$”*. **A threshold is a scale, not a source.** Writing $\Sigma_0 = -a_0R$ and taking $-d\Sigma_0/dR = a_0$ promotes the normalization to a uniform background field acting on every chain, which §5.1 does not license. **The fix:** drop the standalone term. The profile becomes $a = a_N + \sqrt{a_N a_0}$, whose limits are both correct with **no regime switch**, and §7.1's deep-MOND branch becomes right. **The cross-term survives**, because §4.2 uses $a_0R$ as a per-channel *content* measure — legitimate, and equal to $c^2/(2\pi)$ at the horizon, a potential depth of order $c^2$ — whereas §3.2 takes its **gradient** and calls the result a force, which is not. Same expression, two uses, one licensed. *(The opposite repair, $a_0 \to k|\vec a|$, is the **banked negative** of research target #19; it demotes the normalization to an amplitude and is the same conflation from the other side.)* **Applied 2026-09-04 throughout §§3.2, 5.3, 6.3 and 7.1.** Full working: `Note_TheSwitchIsDerived_TheShapeIsInherited.md` §11; `Gravity_TieredClaims_Ledger.md` Staleness #36; target #22.

**Status of $\Sigma_0(R)$ — REWRITTEN 2026-09-04.** $\Sigma_0(R) = -a_0R$ is **not a stability-landscape potential and no gradient of it is taken.** It is a **content normalization**: the depth of horizon content available to a chain at radius $R$, feeding the per-channel horizon content $b_K^{\mathrm{horizon}} = a_0R/N_{\mathrm{horizon}}$ used in §4.2. Read that way it is well-behaved — at the horizon it evaluates to $a_0R_H = c^2/(2\pi)$, a potential depth of order $c^2$, which is what a horizon should have.

**What was removed, and why.** This subsection previously took $-d\Sigma_0/dR = a_0$ and carried a standalone cosmic acceleration of magnitude $a_0$ into §5.3, §6.3 and §7.1. That step is **withdrawn.** `Paper_029` §5.1 gives the cosmic content as $\rho_{\mathrm{cosmic}} = \rho_0(|\vec a|/c)\cos\theta$ with $\rho_0 = cH_0$: the **normalization** $\rho_0$ carries no $|\vec a|$-dependence, while the **response amplitude** $(|\vec a|/c)$ is first order and vanishes for a non-accelerating chain. $a_0 = cH_0/(2\pi)$ is the *projected normalization*, and `Paper_029` §4.4 characterizes it in its own words as *“the **transition acceleration** — the **threshold**”* whose role is set by *“the **ratio** of $a_N$ to $a_0$”*. **A threshold is a scale; it is not a uniform field acting on every chain.** Taking its gradient promoted the normalization to a source.

**This paper's own §3.4 already refuted the term that step produced.** §3.4 rejects the additive combination precisely because $a \to a_0$ in the deep field gives $v^2 = a_0R$, *“empirically refuted by BTFR data”* — and `Paper_031` §(alternatives) independently rejects $a = a_N + a_0$ for the same reason (*“no $M_b$ dependence; slope undefined”*). The standalone term contradicted §3.4 three sections downstream. **Removing it makes the paper self-consistent and aligns it with `Paper_031`.**

*(The opposite repair — restoring $a_0 \to k|\vec a|$ — is a banked **negative** under research target #19: it gives $a \propto GMH_0/R^2$ and breaks MOND. It is the same conflation inverted, demoting the normalization to an amplitude. Full working: `Note_TheSwitchIsDerived_TheShapeIsInherited.md` §11; `Gravity_TieredClaims_Ledger.md` Staleness #36; target #22.)*
...
> **This subsection is what forced the 2026-09-04 rewrite of §3.2.** The argument here — that a standalone $a_0$ gives $a \to a_0$, hence $v^2 = a_0R$, empirically refuted — applies verbatim to the $-d\Sigma_0/dR = a_0$ term that §3.2 previously carried into §§5.3/6.3/7.1. The paper refuted its own term three sections before using it. The term is now removed at source.
...
The bilocal-channel density along the radial direction scales as $1/R$ (substrate-graph radial-projection geometry, anchored by P03 spatial-homogeneity).

> **⚠ THIS IS AN UNNAMED POSTULATE AND IT IS LOAD-BEARING — flagged 2026-09-05 (late).** The clause above is what turns an **$R$-independent** per-channel strain into a logarithm, and therefore what produces $\sqrt{a_N a_0}$ rather than nothing. **It has no name, no audit-table row, and one clause of justification.** Its sibling `P-Bilocal-Count` (§4.2) is named, tiered, and carries falsifier `F-BC`; this one carries none of that and does comparable work. **Stated plainly, it is a claim about how many distinct channels a single chain participates in at one locus, as a function of its distance from a mass** — and *“substrate-graph radial-projection geometry”* is a gesture at a derivation rather than one. **A name is proposed for it in the companion note and deliberately not written here** — a postulate name in a paper increments the corpus census (baselined at 171), so adopting it is AP's call, not a flag's. *(Caught by the census on the same turn: the first draft of this flag carried the name and pushed the count to 172.)* `foundations/Note_StrC_Bridge_2026-09-05.md` §3. The total bilocal contribution to the chain's stability landscape is the radial integral of the bilocal-channel strain content:
...
$$
a(R) = -\frac{d\Sigma}{dR} = -\frac{d\Sigma_N}{dR} - \frac{d\Sigma_{\mathrm{cross}}}{dR} = \frac{GM}{R^2} + \frac{\sqrt{GMa_0}}{R} = a_N + \sqrt{a_N a_0}.
$$

**No regime assumption is needed — REWRITTEN 2026-09-04.** This step previously carried a third term, $-d\Sigma_0/dR = a_0$, and then invoked an explicit *regime assumption* that the bilocal cross-term dominates it. Both are gone: §3.2 no longer takes a gradient of $\Sigma_0$, so there is no third term to dominate, and the deep-MOND limit now follows **as a limit rather than as an assumption**. *(The withdrawn assumption read: “in the joint weak-gradient regime the bilocal-channel cross-term gradient is the dominant contribution, with the pure-source terms subleading.” It was false as stated — with $a_N = \varepsilon a_0$ the terms ordered as $a_N < \sqrt{a_Na_0} < a_0$ for every $\varepsilon<1$, so the standalone term dominated, not the cross-term. See `Gravity_TieredClaims_Ledger.md` Staleness #35.)*

Taking $a_N \ll a_0$ in the two-term result:

In this regime:

$$
a(R) \to \frac{\sqrt{GMa_0}}{R} \qquad (a_N \ll a_0).
$$
...
$$
\boxed{a = a_N + \sqrt{a_N a_0} \;\longrightarrow\; \sqrt{a_N \cdot a_0} \quad (a_N \ll a_0), \text{ given P14.}}
$$

This is the **ED Combination Rule (ECR)**. It is a *downstream structural consequence* of P14 + the joint weak-gradient regime. **Strengthened 2026-09-04:** it previously required an additional regime assumption (that the cross-term dominates two pure-source terms). With $\Sigma_0$ corrected to a content normalization there are only two terms, both limits come out right — $a \to a_N$ for $a_N \gg a_0$, since $\sqrt{a_Na_0}/a_N = \sqrt{a_0/a_N} \to 0$ — and **the deep-MOND branch is a limit, not an assumption.**
...
Standard MOND uses a phenomenological interpolation function $\mu(x)$ with a fitted $a_0$. ED replaces the *fit* but not the *function*: what the substrate supplies is the two-term law $a = a_N + \sqrt{a_N a_0}$, in which **both limits follow as limits** — no regime switch is assumed and none is needed. *(Rewritten 2026-09-04. This previously read “a derived deep-MOND asymptote together with a regime switch that this paper assumes rather than derives”; the switch was needed only to suppress a standalone $a_0$ term that §3.2 should never have produced. With $\Sigma_0$ corrected to a content normalization the term is gone and the assumption with it.)*

> **Update 2026-09-04 — the switch is no longer assumed, and `μ` is now correctly filed as inherited.** Two same-day results change this paragraph's standing. (i) The regime switch is **derived**: `Note_a0_ThresholdFromHorizonCompetition.md` §3 locates it at the competition between the chain's own acceleration horizon and the cosmological one, and `Note_TheSwitchIsDerived_TheShapeIsInherited.md` §3 shows `Γ_cross` collapsing at the nearer of the two from the corpus's own construction, with **no new postulate**. §7.1's assumption is discharged. (ii) The interpolation function is **not an open derivation**: `Note_TheSwitchIsDerived_TheShapeIsInherited.md` §5 identifies `μ`'s shape as the coarse-grained image of the **V5 envelope** `F_V5`, whose functional shape `Paper_090` §7.2 lists as **value-layer inherited** — the same disclaimer `Paper_026` preamble item 3 already carries for the V1 envelope and which no gravity paper had carried for V5. **So `μ` is inherited by construction, not pending.** This is why three separate attempts to derive it were blocked. *Tier: form-IDENTIFIED (M2), conditional on this paper's own bilocal construction.* Within the joint weak-gradient regime the profile is:

$$
a(R) = a_N + \frac{\sqrt{GMa_0}}{R} = a_N + \sqrt{a_N a_0}.
$$
...
**So the honest claim is still narrower than “no interpolation function,” though for a different reason than it was.** *(Rewritten 2026-09-04.)* ED now supplies a **single smooth two-term law with no assumed switch and no fitted parameter** — that much is a genuine economy over a phenomenological $\mu(x)$. What it does **not** supply is a law valid outside the deep field: the same two-term profile saturates as $1-\mu \sim x^{-1/2}$ and is excluded at solar-system accelerations by ephemerides (§7.2), so the crossover in the regime where it matters observationally is `Paper_KM-I` §6.3's, not this paper's. **And the shape itself is inherited, not derived:** the interpolation shape is the coarse-grained image of the V5 envelope, whose functional form `Paper_090` §7.2 lists as value-layer content (`Note_TheSwitchIsDerived_TheShapeIsInherited.md` §5). *The superseded reading — that this is a piecewise profile with an assumed switch — followed from the removed $a_0$ term and no longer applies:* an interpolation function, minus the smoothness and minus the fit. The fit parameter is genuinely gone — $a_0$ is form-derived upstream, subject to `Gravity_TieredClaims_Ledger.md` Staleness #10 on its coefficient — and that is a real economy. The *transition* is not derived here. Deriving the switch is open research (target #19); the horizon-competition mechanism in `Note_a0_ThresholdFromHorizonCompetition.md` §3 is the current candidate, and would supply exactly the P14 on/off that §7.1 assumes. Galaxy-by-galaxy comparison remains empirical-test program.
...
**Transition regime ($a_N \sim a_0$, $R \sim R_{\mathrm{transition}}$):** $a = a_N + \sqrt{a_N a_0}$, both terms comparable; P14 active. *(Corrected 2026-09-04 — this read $a \approx a_N + a_0 + \sqrt{a_N a_0}$ with “all three contributions comparable”; the standalone $a_0$ is removed at source in §3.2.)*
...
> ✅ **RESOLVED 2026-09-04 — this branch is now correct and the contradiction below is gone.** *(Kept as the audit trail. The standalone $a_0$ was removed at source in §3.2, and §6.3's profile is now $a = a_N + \sqrt{a_N a_0}$, whose deep limit is $\sqrt{a_N a_0}$ — so the deep-MOND branch above holds as stated. The rest of this box records the contradiction as it stood before the rewrite.)* **As it stood:** §6.3 gives $a = a_N + a_0 + \sqrt{a_N a_0}$. With $a_N = \varepsilon a_0$ and $\varepsilon \ll 1$ the three terms are $\varepsilon a_0$, $a_0$ and $\sqrt{\varepsilon}a_0$, so the ordering is **always** $a_N < \sqrt{a_N a_0} < a_0$: the **pure cosmic term dominates**, by a factor $1/\sqrt{\varepsilon}$ that grows with depth. The claim above that the cross-term dominates both pure-source terms is **arithmetically false**, and a constant floor $a \to a_0$ gives $v^2 = a_0 R$ — **rising** rotation curves, not flat. **The resolution is §3.2's own flag:** $\Sigma_0 = -a_0 R$ integrates a *response* as though it were a constant *source*. Drop that standalone term and the profile is $a = a_N + \sqrt{a_N a_0}$, whose deep limit is $\sqrt{a_N a_0}$ and this branch is correct. **So §3.2's flag was not an attribution nicety — the deep-MOND result depended on it, and §3.2 has since been rewritten accordingly.** *(Caution: the different repair of restoring `Paper_029`'s $a_0 \to k|a|$ dependence is a banked **negative** — it gives $a \propto GMH_0/R^2$ and breaks MOND. See research target #19.)* Full working: `Note_TheSwitchIsDerived_TheShapeIsInherited.md` §10; `Gravity_TieredClaims_Ledger.md` Staleness #35.
