...
> **Added 2026-09-04 — the elimination is between two endpoints of a continuum.** The argument above refutes a $\Lambda$ tracking the *instantaneous* $H$ and concludes the frozen floor. Those are the ends of a family: writing $\rho_\Lambda \propto H^{2n}$, matter domination gives $\rho_\Lambda \propto a^{-3n}$ and **$w = -1 + n$**, so $n = 1$ is the refuted case and $n = 0$ is the claim. **The argument eliminates $n = 1$ and asserts $n = 0$; the interval between is not addressed here.** Current bounds ($w = -1.03 \pm 0.03$) put $|n| \lesssim 0.03$, so the entire observationally allowed range lies inside that interval — a measured $w = -0.98$ is $n = 0.02$, neither the refuted case nor ED's claim. *This does not weaken F3, which tests $n = 0$ exactly and is the sharper commitment; it means the* argument *for $n = 0$ currently rests on the same saturation-regime reasoning flagged as the residual OPEN above, not on the two-endpoint elimination.* See `Gravity_TieredClaims_Ledger.md` Staleness #24.
