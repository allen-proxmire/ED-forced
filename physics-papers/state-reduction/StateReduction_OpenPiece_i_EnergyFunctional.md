...
> **Note — this section poses the gate; it is RESOLVED below.** This section and "The honest fork" that follows were written *before* the EFE calculation was run. That calculation has since been done (see **"The EFE calculation (2026-07-21) — RESOLVED"** further down): **ED has a full, standard-MOND-like EFE, and Earth's field Newtonizes the collapse to `~3.5×10⁻⁶` — so "The honest fork" resolves to Branch 1 (ground-lab collapse Newtonized, ED ≈ Penrose in the lab).** Read the two sections below as the setup; the resolution supersedes their "not established / open fork" framing.

This is where honesty bites, and it is why this *was* a candidate, not a delivered prediction. MOND is **nonlinear**, so a system's internal dynamics depend on the external gravitational field it is embedded in (the External Field Effect, EFE). A system sitting in an external field `g_ext > a_0` has its internal MOND behavior **Newtonized** — the enhancement is suppressed.
...
3. **CROSS-ARC NOTE ADDED 2026-09-07 — which part of `Paper_029` this leans on, and what that is worth.** This note's EFE rests on `Paper_029` §5.1's **horizon dipole sourced by the chain's total acceleration**. **`Paper_a0z` §3.6 (written 2026-09-07, after this note) partitions `Paper_029` into a RADIAL step and an ANGULAR step and finds every disputed, postulated and analogical row in the angular one** — the dipole machinery. Specifically: *“the dipole is the leading anisotropic mode”* is tiered **D and sound**, but *“cosmic content seen as anisotropic at first order”* is **A — analogy**, and the `1/(2π)` and the `ρ₀ = cH₀` normalisation — both angular — are **disputed** and **postulated** respectively.

    **The conclusion survives, and the direction is what makes that safe.** This note uses the dipole's *structure* (sourced by total acceleration) and `a₀`'s *value*, which is inherited/measured regardless; it does not use the `1/(2π)` or the normalisation. **And the conclusion is a SUPPRESSION.** A shakier dipole argument makes the Newtonisation less certain — it does not resurrect the weapon, because the weapon needed the enhancement to *survive*, and nothing here supplies that. **So this note's “order-of-magnitude structural inference” tier was the right call and is unchanged; what is new is knowing precisely which of `Paper_029`'s steps it inherits, and that the one it inherits is the arc's softer half.**

4. **ED plausibly accounts for the MOND EFE (order-of-magnitude, not a closed derivation).** The horizon-dipole-sourced-by-total-acceleration structure (Paper_029) is a natural ED origin for *why* MOND has an EFE — a useful gravity-arc pointer — but the quantitative suppression here is this note's own linearized estimate, awaiting the nonlinear solve. State it as inference, not theorem.
5. **Two conceptually-distinctive-but-unmeasurable signatures survive.** (a) ED's collapse rate depends on the *ambient gravitational field* (EFE) — a violation of the pure locality Penrose/Diósi assume; (b) it is *anisotropic* — the horizon dipole aligns with `g_ext` (vertical, in a lab), so a superposition displaced along vs. perpendicular to the ambient field would collapse at slightly different rates. Both scale as `√(a_0/g_ext) ~ 10⁻⁶` on Earth: real in principle, unmeasurable in practice.
